...
function findLatestBackup() {
    ...
    const backups = dirs.filter(d => d.startsWith('backup_') && fs.statSync(path.join(FRONTEND_DIR, d)).isDirectory());
...
try {
    ...
    if (fs.existsSync(zipFile)) {
        ...
        execSync(`powershell.exe -NoProfile -NonInteractive -Command "Expand-Archive -Path '${zipFile}' -DestinationPath '${FRONTEND_DIR}' -Force"`, { stdio: 'inherit' });
    } else {
