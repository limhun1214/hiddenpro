[2026-04-01] 추천 목록 가입 즉시 표시 — referral/page.tsx 데이터 소스를 referral_rewards → users.referred_by 기반 get_my_referrals RPC로 전환, 통계 카운트·목록 UI 업데이트 / frontend/src/app/referral/page.tsx / DB변경: get_my_referrals RPC 추가 필요 (SQL 별도 제공)
