...
async function run() {
    ...
    const [proResult, catResult] = await Promise.all([
        supabase.from('pro_profiles').select('pro_id, current_cash, bonus_cash').eq('is_accepting_requests', true).limit(1).single(),
        supabase.from('categories').select('id').limit(1).single(),
    ]);

    if (proResult.error || !proResult.data) {
        console.error('❌ 활성 고수 계정을 찾을 수 없습니다:', proResult.error?.message);
        process.exit(1);
    }
    if (catResult.error || !catResult.data) {
        console.error('❌ categories 테이블에서 카테고리를 찾을 수 없습니다:', catResult.error?.message);
        process.exit(1);
    }

    const testProId: string = proResult.data.pro_id;
    const testCategoryId: string = catResult.data.id;
    const originalCash = { bonus: proResult.data.bonus_cash ?? 0, real: proResult.data.current_cash ?? 0 };

    // 고객 ID 조회
    const { data: custData, error: custErr } = await supabase
        .from('users').select('user_id').eq('role', 'CUSTOMER').eq('status', 'ACTIVE').limit(1).single();
    if (custErr || !custData) {
    ...
    for (const tc of TEST_CASES) {
        ...
        console.log(`  입력: bonus=${tc.bonusBefore}, real=${tc.realBefore}, 차감=${tc.deductAmount}`);
        console.log(`  예상: bonus=${tc.expectedBonus}, real=${tc.expectedReal}, ledger ${tc.expectedLedgerCount}건 ${tc.expectedLedgerTypes.join('+')}`);
        ...
        try {
            ...
            const { error: setErr } = await supabase
                ...
                .eq('pro_id', testProId);
            if (setErr) { console.log(`  ❌ 캐시 강제 설정 실패: ${setErr.message}`); allPassed = false; continue; }
            ...
            const { data: reqData, error: reqErr } = await supabase
                ...
                .select('request_id')
                .single();
            if (reqErr || !reqData) { console.log(`  ❌ 요청서 생성 실패: ${reqErr?.message}`); allPassed = false; continue; }
            testRequestId = reqData.request_id;

            // STEP 3: RPC 호출
            const { error: rpcErr } = await supabase.rpc('send_quote_and_deduct_cash', {
                p_pro_id: testProId,
            ...
            if (rpcErr) { console.log(`  ❌ RPC 실패: ${rpcErr.message}`); allPassed = false; continue; }

            // STEP 4: DB 결과 조회 (N+1 방지: 동시 조회)
            const [proAfter, ledgerAfter] = await Promise.all([
                supabase.from('pro_profiles').select('current_cash, bonus_cash').eq('pro_id', testProId).single(),
                supabase.from('cash_ledger').select('tx_type, amount').eq('reference_id', testRequestId).order('created_at', { ascending: true }),
            ]);
            ...
            const ledgerTypesOk = tc.expectedLedgerTypes.every(t => actualLedger.some((r: any) => r.tx_type === t));
            const passed = bonusOk && realOk && ledgerCountOk && ledgerTypesOk;

            if (!passed) allPassed = false;

            console.log(`  결과: bonus=${actualBonus} ${bonusOk ? '✅' : `❌(예상 ${tc.expectedBonus})`}, real=${actualReal} ${realOk ? '✅' : `❌(예상 ${tc.expectedReal})`}`);
            console.log(`  원장: ${actualLedger.length}건 ${actualLedger.map((r: any) => `${r.tx_type}(${r.amount})`).join(', ')} ${ledgerCountOk && ledgerTypesOk ? '✅' : '❌'}`);
            console.log(passed ? `  ✅ PASS` : `  ❌ FAIL`);

        } finally {
            // STEP 5: 테스트 데이터 정리
            if (testRequestId) {
                await supabase.from('cash_ledger').delete().eq('reference_id', testRequestId);
                await supabase.from('match_quotes').delete().eq('request_id', testRequestId);
                await supabase.from('match_requests').delete().eq('request_id', testRequestId);
            }
        }
    }
    ...
    // 최종: 고수 캐시 원복
    console.log(`\n─────────────────────────────────────`);
    console.log(`🧹 고수 캐시 원복 중... (bonus=${originalCash.bonus}, real=${originalCash.real})`);
    await supabase.from('pro_profiles')
        .update({ bonus_cash: originalCash.bonus, current_cash: originalCash.real })
        .eq('pro_id', testProId);
    console.log(`🧹 원복 완료`);

    console.log(`\n═════════════════════════════════════`);
    console.log(allPassed ? `✅ 전체 테스트 통과 — 보너스 우선 차감 로직 정상 동작` : `❌ 일부 테스트 실패 — 결과 확인 필요`);
    console.log(`═════════════════════════════════════\n`);
}
