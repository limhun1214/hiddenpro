...
async function main() {
    ...
    const { rows } = await client.query(`SELECT id, name, depth1, depth2 FROM public.categories`);
    ...
    for (const [d1Name, d1Rows] of Object.entries(byDepth1)) {
        ...
        for (let i = 0; i < d1Rows.length; i++) {
            ...
            await client.query(`UPDATE public.categories SET sort_order = $1 WHERE id = $2`, [finalOrder, d1Rows[i].id]);
        }
    }
