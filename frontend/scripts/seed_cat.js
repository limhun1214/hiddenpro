...
const SERVICE_CATEGORIES = {
  "이사/청소": {
    "가사/메이드": ["파트타임 가사도우미", "요리 도우미", "육아/베이비시터"],
    "집 청소": [
      "거주/정기 청소",
      "이사/입주 딥클리닝",
      "콘도/수영장 유지보수 청소",
    ...
    "특수 청소 및 방역": ["흰개미 퇴치", "일반 해충 방역", "곰팡이/악취 제거"],
    "이사 및 운송": ["용달/화물 운송", "가정이사", "사무실/상업공간 이사"],
    "폐기물 처리": ["대형 폐기물 수거 및 처리"],
  },
  "설치/수리": {
    "수도/배관": [
      "(긴급) 누수 및 수도관 수리",
...
async function run() {
  ...
  try {
    ...
    await client.query(
      `ALTER TABLE match_requests ALTER COLUMN category_id DROP NOT NULL;`,
    );
    ...
    console.log(
      "Remapping match_requests.category_id based on service_type...",
    );
    await client.query(`
