...
async function run() {
    ...
    try {
        ...
        if (settingErr || !settingData) {
            console.error('❌ platform_settings 에서 max_quotes_per_request 를 찾을 수 없습니다:', settingErr?.message);
            process.exit(1);
        }
        ...
        const [custResult, catResult] = await Promise.all([
            supabase.from('users').select('user_id').eq('role', 'CUSTOMER').eq('status', 'ACTIVE').limit(1).single(),
            supabase.from('categories').select('id').limit(1).single()
        ]);

        if (custResult.error || !custResult.data) {
            console.error('❌ 활성 고객 계정을 찾을 수 없습니다:', custResult.error?.message);
            process.exit(1);
        }
        ...
        if (countErr) {
            ...
            if (count === originalValue) {
                console.log(`   → RPC 내부에 v_max_quotes 가 하드코딩(${originalValue})되어 있어 platform_settings 변경이 적용되지 않는 것으로 판단됩니다.`);
                console.log(`   → 수정 방법: send_quote_and_deduct_cash RPC 에서 platform_settings 테이블의 max_quotes_per_request 값을 동적으로 읽도록 수정 필요.`);
            }
        }
        ...
        if (originalValue !== null) {
            const { error: restoreErr } = await supabase.rpc('update_platform_setting', {
                p_key: SETTING_KEY,
