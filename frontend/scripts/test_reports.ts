...
async function main() {
    ...
    const { data: customers } = await query('users?role=eq.CUSTOMER&limit=1&select=user_id,nickname,name,status');
    const { data: proUsers } = await query('users?role=eq.PRO&limit=1&select=user_id,nickname,name,status');
    ...
    try {
        ...
        if (verifyUser?.[0]?.status === 'SUSPENDED' && verifyUser[0].suspension_type === 'temporary') {
        ...
        if (cleanup.reportId) {
            ...
            if (verifyReport?.[0]?.status === 'reviewed' && verifyReport[0].admin_note) {
