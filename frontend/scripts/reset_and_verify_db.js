...
const connectionString = process.env.DIRECT_URL || "postgresql://postgres.sjhemxejhyztbsctkqvb:Wkaqls191214@aws-1-ap-northeast-2.pooler.supabase.com:5432/postgres";
...
async function runResetAndVerify() {
    ...
    `;
        ...
        const tablesToCheck = [
            'notifications', 'reviews', 'chat_messages',
            'chat_rooms', 'match_quotes', 'match_requests'
        ];
        ...
        const profilesRes = await client.query('SELECT SUM(current_cash) as total_cash FROM Pro_Profiles');
        const totalCash = profilesRes.rows[0].total_cash || 0;

        const ledgerRes = await client.query("SELECT SUM(amount) as total_bonus FROM Cash_Ledger WHERE tx_type = 'BONUS'");
        ...
        if (totalCash === totalBonus) {
            console.log("✅ 검증 B 통과: Pro_Profiles와 Cash_Ledger의 총합이 1:1로 일치합니다.");
        } else {
