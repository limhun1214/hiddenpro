...
(async () => {
  ...
  const [customerBtn] = await page.$$(
    '::-p-xpath(//h2[contains(., "고객으로 시작하기")]/ancestor::button)',
  );
  if (customerBtn) {
    ...
    const h2Elements = await page.$$eval("h2", (els) =>
      els.map((el) => el.textContent),
    );
    const chatH2 = h2Elements.find((text) => text.includes("대화방")) || "N/A";

    const navElements = await page.$$eval("nav span", (els) =>
      els.map((el) => el.textContent),
    );
    const hasWallet = navElements.some((text) => text.includes("지갑"))
      ? "Yes"
      : "No";

    const roleLog =
      logs.find((l) => l.includes("Current Verified Role:")) || "N/A";
  ...
  const [proBtn] = await page.$$(
    '::-p-xpath(//h2[contains(., "고수로 시작하기")]/ancestor::button)',
  );
  if (proBtn) {
    await proBtn.click();
    await delay(3000);
    ...
    const h2Elements2 = await page.$$eval("h2", (els) =>
      els.map((el) => el.textContent),
    );
    const chatH22 =
      h2Elements2.find((text) => text.includes("대화방")) || "N/A";

    const navElements2 = await page.$$eval("nav span", (els) =>
      els.map((el) => el.textContent),
    );
    const hasWallet2 = navElements2.some((text) => text.includes("지갑"))
      ? "Yes"
      : "No";
