...
async function main() {
    ...
    await client.query(`ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS sort_order INTEGER DEFAULT 999;`);
    ...
    for (const p of parents) {
        ...
        await client.query(`UPDATE public.categories SET sort_order = $1 WHERE id = $2`, [baseOrder, p.id]);
    }
    ...
    for (const [pId, childs] of Object.entries(childrenByParent)) {
        ...
        for (let i = 0; i < childs.length; i++) {
            await client.query(`UPDATE public.categories SET sort_order = $1 WHERE id = $2`, [baseOrder + i + 1, childs[i].id]);
        }
    }
