...
content = content.replace(/\breturn alert\((.*?)\);/g, "{ await showAlert($1); return; }");
...
content = content.replace(/if \(\!window\.confirm\(([^)]*(\([^)]*\)[^)]*)*)\)\) return;/g, "if (!await showConfirm($1)) return;");

content = content.replace(/\bwindow\.confirm\(([^)]*(\([^)]*\)[^)]*)*)\)/g, "await showConfirm($1)");

// 5. window.prompt -> showPrompt
content = content.replace(/\bwindow\.prompt\(([^)]*(\([^)]*\)[^)]*)*)\)/g, "await showPrompt($1)");
...
// E.g. onClick={() => { ... await ... }}
content = content.replace(/onClick=\{\(\) => (\{[^}]*(?:await show)[^}]*\})\}/g, "onClick={async () => $1}");
// E.g. onClick={() => await ... }
content = content.replace(/onClick=\{\(\) => (await show.*?)\}/g, "onClick={async () => $1}");
