...
async function checkAndFixRealtime() {
  ...
  try {
    ...
    const hasMatchRequests = pubTables.some(
      (r) => r.tablename === "match_requests",
    );
    console.log(
      "\n🔍 match_requests 포함 여부:",
      hasMatchRequests ? "✅ YES" : "❌ NO",
    );
    ...
    console.log(
      "\n🔍 match_requests REPLICA IDENTITY:",
      ri === "f" ? "FULL" : ri === "d" ? "DEFAULT" : ri,
    );

    if (ri !== "f") {
      console.log(
        "🔧 REPLICA IDENTITY를 FULL로 변경합니다 (UPDATE 이벤트에 old/new 모두 전송)...",
      );
