...
for (let i = 0; i < lines.length; i++) {
  ...
  for (let j = 0; j < closeDivs; j++) {
    if (tagStack.length === 0) {
      ...
      if (foundIdx !== -1) {
        ...
        console.log(
          `ERROR: Mismatch at line ${i + 1}. Expected div but no open div found`,
        );
      }
    }
  }
  ...
  for (let j = 0; j < openFrags; j++)
    tagStack.push({ tag: "fragment", line: i + 1 });
  ...
  for (let j = 0; j < closeFrags; j++) {
    if (tagStack.length === 0) {
      ...
      if (foundIdx !== -1) {
        ...
        console.log(
          `ERROR: Mismatch at line ${i + 1}. Expected fragment but no open fragment found`,
        );
      }
    }
  }
