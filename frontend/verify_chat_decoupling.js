...
async function test() {
    ...
    const { data: request } = await supabase.from('match_requests').select('*').eq('status', 'OPEN').limit(1).single();
    if (!request) return console.log('No open requests');

    // 2. Get a pro user to act as quoter
    const { data: pro } = await supabase.from('users').select('*').eq('role', 'PRO').limit(1).single();
    ...
    const { data: verifyReq } = await supabase.from('match_requests').select('status').eq('request_id', requestId).single();
