...
function doBackup() {
    ...
    fs.writeFileSync(path.join(backupDir, 'db_full_backup.json'), JSON.stringify({ schema: schemaContent }, null, 2));
    ...
    try {
        ...
        execSync(`tar -a -c -f "${zipPath}" --exclude=node_modules --exclude=.next --exclude=.git --exclude=backup_* *`, {
            cwd: __dirname,
            stdio: 'inherit'
        });
