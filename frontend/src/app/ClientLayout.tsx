...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        const checkProProfile = async () => {
            const { data, error } = await supabase
                .from('pro_profiles')
                .select('phone, is_phone_verified, intro, detailed_intro, region, services')
                .eq('pro_id', userId)
                .single();
            ...
            const hasDetailedIntro = !!data.detailed_intro && data.detailed_intro.trim().length > 0;
            ...
            setIsProProfileComplete(hasPhone && hasIntro && hasDetailedIntro && hasRegion && hasServices);
        };
        checkProProfile();
    }, [userId, isProUser]);
        ...
        const initAuth = async () => {
            ...
            try {
                ...
                if (authError || !sessionUser) {
                    if (isMounted) { setIsProUser(false); setUserId(null); }
                    return;
                }
                ...
                if (!dbError && userData && String(userData.status).toUpperCase() === 'SUSPENDED') {
                ...
                if (isPro) {
                    const { data, error } = await supabase
                        ...
                        .single();
                    if (!error && data && isMounted) setWalletBalance((data.current_cash || 0) + (data.bonus_cash || 0));
                }
                ...
                if (!notifError && count !== null && isMounted) setUnreadNotifsCount(count);
                ...
                const { count: quoteCount, error: quoteError } = await supabase
                    .from('match_quotes')
                    .select('quote_id, match_requests!inner(customer_id)', { count: 'exact', head: true })
                    .eq('match_requests.customer_id', sessionUser.id)
                    .eq('is_read', false);
                if (!quoteError && quoteCount !== null && quoteCount > 0 && isMounted) setHasNewQuotes(true);
                ...
                if (myRoomIds.length > 0) {
                    const { count: chatCnt, error: chatError } = await supabase
                        ...
                        .eq('is_read', false);
                    if (!chatError && chatCnt !== null && isMounted) setUnreadChatsCount(chatCnt);
                } else {
    ...
    */
    ...
    useEffect(() => {
        ...
        const notifChannel = supabase.channel('gnb-notif-' + userId)
            .on('postgres_changes',
                ...
                (payload: any) => {
                    ...
                    window.dispatchEvent(new CustomEvent('notification-inserted', { detail: payload.new }));
        ...
        const chatChannel = supabase.channel('gnb-chat-' + userId)
            .on('postgres_changes',
                {
                    event: 'INSERT',
                    schema: 'public',
        ...
        const handleWalletUpdate = async () => {
            const { data, error } = await supabase
                .from('pro_profiles').select('current_cash, bonus_cash').eq('pro_id', userId).single();
            if (!error && data) setWalletBalance((data.current_cash || 0) + (data.bonus_cash || 0));
        };
        ...
        const handleBadgeSync = (e: Event) => {
            ...
            if (d.unreadNotifsCount !== undefined) setUnreadNotifsCount(d.unreadNotifsCount);
        };
        ...
        return () => {
            ...
            window.removeEventListener('force-requests-badge', handleForceRequestsBadge);
    ...
    const isChatRoom = currentPath.startsWith('/chat') && currentPath !== '/chat' && currentPath !== '/chat/';
    ...
    const rightPanelClasses = "flex flex-col w-full flex-1 min-h-screen lg:min-h-0 lg:h-full bg-white relative";

    return (
        <ToastProvider>
            <NavStateContext.Provider value={{ unreadNotifsCount, unreadChatsCount, hasNewQuotes, hasNewRequests, isProUser, walletBalance, isProProfileComplete, setShowProfileIncompleteModal }}>
                <div className={rootContainerClasses}>
                    {!isSpecialPage && <BrandSidePanel />}

                    <div className={isSpecialPage ? "flex flex-col w-full min-h-screen" : rightPanelClasses}>
                        {!isSpecialPage && <PCTopNav />}

                        {isCheckingAuth && (currentPath === '/pro' || currentPath.startsWith('/pro/')) ? (
                            <div className="flex-1 flex flex-col items-center justify-center min-h-screen">
                                <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div>
                            </div>
                        ) : (
                            <>
                                <main className={`flex-1 flex flex-col w-full ${!isSpecialPage ? 'max-w-4xl mx-auto lg:overflow-y-auto custom-scrollbar' : ''} ${!hideNavBar ? 'pb-16' : ''}`}>
                                    {children}
                                </main>
                                                ...
                                                return (
                                                    <li key={item.label} className="flex-1">
                                                        <Link
                                                            ...
                                                            className={`flex flex-col items-center justify-center w-full h-full space-y-1 transition-colors ${isActive ? 'text-blue-600 font-bold' : 'text-gray-400 hover:text-blue-400 font-medium'
                                                                }`}
                                                        >
                                                            ...
                                                            <span className={`text-[10px] ${isActive ? 'scale-110 transition-transform' : ''}`}>
                                                                {item.label}
                                                            </span>
                                                        </Link>
                                                    </li>
