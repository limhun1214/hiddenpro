...
function ProReviewCard({
  ...
  review: any;
  customerProfile?: { name: string; avatar: string | null };
  onClickViewQuote: (review: any) => void;
  t: any;
}) {
    ...
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-3">
      <div className="flex justify-between items-center w-full">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-800 text-sm">
            {customerProfile?.name || t("proReviews.unknownCustomer")}
          </span>
        </div>
      ...
      <div className="flex items-center gap-1">
        {[1, 2, 3, 4, 5].map((s) => (
          <span
            key={s}
            className={`text-lg ${s <= review.rating ? "text-yellow-400" : "text-gray-200"}`}
          >
            ★
          </span>
        ))}
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: rawReviews, error: reviewError } = await supabase
        .from("reviews")
        .select(
          "review_id, rating, comment, created_at, customer_id, room_id, pro_id",
        )
        .eq("pro_id", proId)
        .eq("is_hidden", false)
      ...
      if (!reviewError && rawReviews) {
        const customerIds = Array.from(
          new Set(rawReviews.map((r: any) => r.customer_id).filter(Boolean)),
        );
              ...
              cMap[c.user_id] = {
                name:
                  c.nickname && c.nickname.trim() !== ""
                    ? c.nickname
                    : c.name || t("proReviews.unknownCustomer"),
                ...
                if (requestsData) {
                  enrichedReviews = rawReviews.map((review) => {
                    const room = chatRooms.find(
                      (r) => r.room_id === review.room_id,
                    );
                    if (!room) return review;

                    const request = requestsData.find(
                      (req) => req.request_id === room.request_id,
                    );
                    if (!request) return review;

                    let quote = null;
                    if (request.match_quotes) {
                      const quotes = Array.isArray(request.match_quotes)
                        ? request.match_quotes
                        : [request.match_quotes];
                      quote = quotes.find(
                        (q: any) => q.pro_id === review.pro_id,
                      );
                    }
        ...
        <div className="flex items-center gap-2">
          <div className="flex">
            {[1, 2, 3, 4, 5].map((s) => (
              <span
                key={s}
                className={`text-3xl ${s <= Math.round(avgRating) ? "text-yellow-400" : "text-gray-200"}`}
              >
                ★
              </span>
            ))}
