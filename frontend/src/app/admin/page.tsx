...
const txLabel = (t: string, locale: string = "en") => {
  const map: Record<string, Record<string, string>> = {
    en: {
      CHARGE: "Credits top-up",
      DEDUCT_QUOTE: "Credits used",
      REFUND: "Credits refund",
      BONUS: "Bonus credits",
      ADMIN_CHARGE: "Admin top-up",
      ADMIN_REFUND: "Admin refund",
      ADMIN_DEDUCT: "Admin deduction",
      BONUS_REFUND: "Unread refund",
      ADMIN_BONUS_CHARGE: "Admin bonus",
      ADMIN_BONUS_REFUND: "Admin bonus deduction",
    },
    ko: {
      CHARGE: "충전",
      DEDUCT_QUOTE: "견적 차감",
      ...
      ADMIN_DEDUCT: "관리자 차감",
      BONUS_REFUND: "미열람 보상",
      ADMIN_BONUS_CHARGE: "관리자 보너스 지급",
      ADMIN_BONUS_REFUND: "관리자 보너스 차감",
    },
  };
...
const txDesc = (
  ...
  if (desc) return desc;
  if (
    ["CHARGE", "ADMIN_CHARGE", "ADMIN_BONUS_CHARGE", "BONUS"].includes(txType)
  )
    return locale === "ko" ? "캐시 충전" : "Credits top-up";
  if (txType === "DEDUCT_QUOTE")
    return locale === "ko" ? "견적 발송" : "Quote sent";
  if (["REFUND", "ADMIN_REFUND", "BONUS_REFUND"].includes(txType))
    return locale === "ko" ? "캐시 환불" : "Credits refund";
  return "-";
};
type LedgerCategory =
  | "all"
...
const fmtDate = (d: string, locale: string = "en") =>
  new Date(d).toLocaleString(locale === "ko" ? "ko-KR" : "en-US", {
    year: "2-digit",
    month: "2-digit",
    day: "2-digit",
    hour: "2-digit",
...
function AdminDashboardPageContent() {
  ...
  const [userMenuOpen, setUserMenuOpen] = useState(
    ["pro", "customer", "admin_mgmt"].includes(initialTab),
  );
  const [group1Open, setGroup1Open] = useState(
    ["dashboard", "search_logs"].includes(initialTab) ||
      initialTab === "dashboard",
  );
  const [group3Open, setGroup3Open] = useState(
    ["ledger", "payout"].includes(initialTab),
  );
  const [group4Open, setGroup4Open] = useState(
    ["quotes", "reviews"].includes(initialTab),
  );
  const [group5Open, setGroup5Open] = useState(
    ["inquiries", "abuse", "reports"].includes(initialTab),
  );
  const [group6Open, setGroup6Open] = useState(
    ["categories", "cms", "audit_log", "settings"].includes(initialTab),
  );
  ...
  const [drilldownFilter, setDrilldownFilter] = useState<
    "all" | "DEDUCT_QUOTE" | "CHARGE" | "REFUND"
  >("all");
  ...
  const [proFilter, setProFilter] = useState<
    "all" | "verified" | "unverified" | "suspended" | "deleted"
  >("all");
  ...
  const [csContactInfo, setCsContactInfo] = useState<{
    customer: any;
    pro: any;
  } | null>(null);
  ...
  // CS 채팅 내역 섹션
  const [csChatRoomId, setCsChatRoomId] = useState<string | null | undefined>(
    undefined,
  );
  ...
  const [csChatModal, setCsChatModal] = useState<{
    proName: string;
    customerName: string;
    customerId: string;
  } | null>(null);
  ...
  // CS 견적 고수 상세 팝업
  const [proDetailModal, setProDetailModal] = useState<{
    proId: string;
  } | null>(null);
  const [proDetailData, setProDetailData] = useState<any>(null);
  const [proDetailLoading, setProDetailLoading] = useState(false);
  const [proDetailTab, setProDetailTab] = useState<
    "info" | "ledger" | "quotes" | "reviews"
  >("info");
  const [proDetailTabData, setProDetailTabData] = useState<{
    ledger?: any[];
    quotes?: any[];
    reviews?: any[];
  }>({});
  ...
  const [editingSupportPage, setEditingSupportPage] = useState<any | null>(
    null,
  );
  ...
  const [categoryDepth1Filter, setCategoryDepth1Filter] =
    useState<string>("all");
  const [categoryDepth2Filter, setCategoryDepth2Filter] =
    useState<string>("all");
  ...
  const [unflagConfirmModal, setUnflagConfirmModal] = useState<{
    userId: string;
    nickname: string;
  } | null>(null);
  const ABUSE_PAGE_SIZE = 50;
  // ── [확장] 제재 이력 타임라인 ──
  const [abuseTimeline, setAbuseTimeline] = useState<{
    user: any;
    logs: any[];
    loading: boolean;
  } | null>(null);
  ...
  const [reportSort, setReportSort] = useState<"latest" | "most_reported">(
    "latest",
  );

  const [payoutRequests, setPayoutRequests] = useState<any[]>([]);
  const [payoutLoading, setPayoutLoading] = useState(false);
  const [payoutActionLoading, setPayoutActionLoading] = useState<string | null>(
    null,
  );
  const [payoutNote, setPayoutNote] = useState<{ [key: string]: string }>({});
  const [payoutFilter, setPayoutFilter] = useState<
    "all" | "PENDING" | "HELD" | "APPROVED" | "REJECTED"
  >("all");
  ...
  const [unreadCounts, setUnreadCounts] = useState<{
    ...
    reports: number;
    payout: number;
  }>({ inquiries: 0, abuse: 0, reports: 0, payout: 0 });
  ...
  const playAlertSound = useCallback((isUrgent: boolean) => {
    console.log(
      "[AdminAlert] playAlertSound called, hasInteracted:",
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (!isMounted) return;
        const allowedAdminRoles: AdminRole[] = [
          "ADMIN",
          "ADMIN_OPERATION",
          "ADMIN_VIEWER",
        ];
  ...
  useEffect(() => {
    ...
    events.forEach((e) =>
      window.addEventListener(e, markInteracted, { once: false }),
    );
    return () => {
  ...
  useEffect(() => {
    ...
    const fetchUnreadCounts = async () => {
      try {
        ...
        if (newCounts.payout > prev.payout)
          alerts.push({
            key: "payout",
            label: adminLocale === "ko" ? "새 출금 요청" : "New payout request",
            count: newCounts.payout - prev.payout,
            urgent: true,
          });
        if (newCounts.reports > prev.reports)
          alerts.push({
            key: "reports",
            label: adminLocale === "ko" ? "새 신고 접수" : "New report filed",
            count: newCounts.reports - prev.reports,
            urgent: true,
          });
        if (newCounts.inquiries > prev.inquiries)
          alerts.push({
            key: "inquiries",
            label: adminLocale === "ko" ? "새 1:1 문의" : "New inquiry",
            count: newCounts.inquiries - prev.inquiries,
            urgent: false,
          });
        if (newCounts.abuse > prev.abuse)
          alerts.push({
            key: "abuse",
            label:
              adminLocale === "ko" ? "새 어뷰징 감지" : "New abuse detected",
            count: newCounts.abuse - prev.abuse,
            urgent: false,
          });
  ...
  const loadDashboard = useCallback(async () => {
    ...
    let c24 = 0,
      d24 = 0;
    (l24 || []).forEach((t) => {
      if (t.amount > 0) c24 += Number(t.amount);
      else d24 += Math.abs(Number(t.amount));
    });
    setStats({
      customers: cc || 0,
      pros: pc || 0,
  ...
  const loadLedger = useCallback(
    async (
      ...
      try {
        ...
        if (category === "REFUND") {
          q = q.in("tx_type", refundTypes);
        } else if (category !== "all") {
          q = q.eq("tx_type", category);
        }
        // Period filter
        if (period !== "all") {
          const days =
        ...
        const proIds = Array.from(
          new Set(rows.map((r) => r.pro_id).filter(Boolean)),
        );
        let proMap: Record<
          string,
        ...
        if (search) {
          ...
          enriched = enriched.filter((r) =>
            [r.proName, r.proEmail, r.proPhone, r.pro_id].some((v) =>
              (v || "").toLowerCase().includes(q2),
            ),
          );
        ...
        if (category === "REFUND") {
          statsQ = statsQ.in("tx_type", refundTypes);
        } else if (category !== "all") {
          statsQ = statsQ.eq("tx_type", category);
        }
        if (period !== "all") {
          const days =
            period === "today"
        ...
        let tpb = 0;
        (allProCash || []).forEach((p: any) => {
          tpb += Number(p.current_cash || 0);
        });
  ...
  const loadPros = useCallback(async () => {
    ...
    const { data: allUsers } = await supabase
      .from("users")
      .select("user_id, name, nickname, phone, status, created_at")
      .eq("role", "PRO");
  ...
  const loadCustomers = useCallback(async () => {
    ...
    setCustomers(
      (data || []).map((c) => ({
        ...c,
        email: emailMap[c.user_id] || c.name || "",
      })),
    );
  }, []);
  ...
  const loadAdmins = useCallback(async () => {
    const { data } = await supabase
      .from("users")
      .select("user_id, name, nickname, role, status, created_at")
      .in("role", ["ADMIN", "ADMIN_OPERATION", "ADMIN_VIEWER"])
  ...
  const handlePromoteAdmin = async () => {
    if (!promoteEmail.trim()) {
      setModal({
        type: "error",
        title: adminLocale === "ko" ? "입력 오류" : "Input Error",
        message:
          adminLocale === "ko"
            ? "이메일을 입력하세요."
    ...
    try {
      ...
      if (findErr || !targetUser) {
        setModal({
          type: "error",
          title: adminLocale === "ko" ? "계정 없음" : "Account Not Found",
          message:
            adminLocale === "ko"
              ? "존재하지 않는 계정입니다."
      ...
      if (
        ["ADMIN", "ADMIN_OPERATION", "ADMIN_VIEWER"].includes(targetUser.role)
      ) {
        setModal({
          type: "error",
          title: adminLocale === "ko" ? "승급 불가" : "Cannot Promote",
          message:
            adminLocale === "ko"
              ? "이미 관리자 계정입니다."
              : "Already an admin account.",
        });
        return;
      }
      if (targetUser.user_id === currentAdminId) {
        setModal({
          type: "error",
          title: adminLocale === "ko" ? "승급 불가" : "Cannot Promote",
          message:
      ...
      if (rpcError) {
        setModal({
          type: "error",
          title: adminLocale === "ko" ? "승급 실패" : "Promotion Failed",
      ...
      setModal({
        type: "success",
        title: adminLocale === "ko" ? "승급 완료" : "Promotion Complete",
        message:
          adminLocale === "ko"
            ? `${promoteEmail} 계정이 ${promoteRole === "ADMIN_OPERATION" ? "운영 관리자" : "뷰어"}로 승급되었습니다.`
            : `${promoteEmail} has been promoted to ${promoteRole === "ADMIN_OPERATION" ? "Operations Admin" : "Viewer"}.`,
      });
      setPromoteEmail("");
      loadAdmins();
    } finally {
  ...
  const handleRevokeAdmin = async (
    ...
    try {
      ...
      if (rpcError) {
        setModal({
          type: "error",
          title: adminLocale === "ko" ? "권한 회수 실패" : "Revoke Failed",
          message: rpcError.message,
        });
        return;
      }
      ...
      setModal({
        type: "success",
        title: adminLocale === "ko" ? "권한 회수 완료" : "Revoke Complete",
        message:
          adminLocale === "ko"
            ? "해당 계정의 권한이 회수되고 계정이 정지되었습니다."
            : "The account has been revoked and suspended.",
      });
      loadAdmins();
    } finally {
  ...
  const loadRequests = useCallback(async () => {
    ...
    setRequests(
      (data || []).map((r: any) => {
        ...
        let matchedProName = "";
        let matchedPrice = 0;
        const acceptedQuote = (r.match_quotes || []).find(
          (mq: any) => mq.status === "ACCEPTED",
        );
        if (acceptedQuote) {
          matchedProName =
            acceptedQuote.pro?.nickname ||
            acceptedQuote.pro?.name ||
            acceptedQuote.pro_id?.slice(0, 8);
          matchedPrice = acceptedQuote.price || 0;
        } else {
          matchedProName =
            r.status === "OPEN"
              ? adminLocale === "ko"
                ? `대기 중 (${(r.match_quotes || []).length}명 입찰)`
                : `Pending (${(r.match_quotes || []).length} bids)`
              : "-";
        }
  ...
  const loadReviews = useCallback(async () => {
    const { data } = await supabase
      .from("reviews")
      .select(
        "review_id, rating, comment, created_at, customer_id, pro_id, room_id, is_hidden, is_featured_on_main",
      )
      .order("created_at", { ascending: false })
      .limit(100);
    ...
    (usersData || []).forEach((u) => {
      nameMap[u.user_id] = u.nickname || u.name || u.user_id.slice(0, 8);
    });
    setReviews(
      (data || []).map((r) => ({
        ...r,
        customerName:
          nameMap[r.customer_id] ||
          (adminLocale === "ko" ? "고객" : "Customer"),
        proName: nameMap[r.pro_id] || (adminLocale === "ko" ? "고수" : "Pro"),
      })),
    );
  }, []);
  ...
  const loadUserDetail = useCallback(
    async (userId: string, role: "PRO" | "CUSTOMER") => {
      setUserDetail({ userId, role });
      ...
      det.user = uData;
      if (role === "PRO") {
        const { data: allProfiles } = await supabase.rpc(
          "get_all_pro_profiles",
        );
        const pData =
          (allProfiles || []).find((p: any) => p.pro_id === userId) || null;
        det.profile = pData;
      ...
      if (role === "CUSTOMER") {
        const { data: reqs } = await supabase
          .from("match_requests")
          .select(
            "request_id, status, service_type, region, created_at, match_quotes(quote_id, pro_id, price, status, created_at)",
          )
        ...
        const { data: quotes } = await supabase
          .from("match_quotes")
          .select(
            "quote_id, price, description, status, created_at, request_id",
          )
          .eq("pro_id", userId)
          .order("created_at", { ascending: false })
          .limit(30);
      ...
      if (role === "CUSTOMER") {
        const { data: rvs } = await supabase
          .from("reviews")
          .select("review_id, rating, comment, created_at, pro_id")
          .eq("customer_id", userId)
          .order("created_at", { ascending: false });
        det.reviews = rvs || [];
      } else {
  ...
  const loadInquiries = useCallback(
    async (page = 1) => {
      ...
      if (inquiriesFilter !== "all")
        countQ = countQ.eq("status", inquiriesFilter);
      if (inquiriesCategory !== "all")
        countQ = countQ.eq("category", inquiriesCategory);
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        (adminLocale === "ko" ? "저장 실패: " : "Save failed: ") + e.message,
      );
    }
  };
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      if ((reqUse && reqUse.length > 0) || (proUse && proUse.length > 0)) {
        alert(
          adminLocale === "ko"
            ? "현재 견적 요청이나 고수 프로필에서 사용 중인 카테고리이므로 삭제할 수 없습니다. 대신 [활성 상태]를 '운영 중지'로 변경해 주세요."
            : "This category is in use by quote requests or pro profiles and cannot be deleted. Please set its status to 'Inactive' instead.",
        );
      ...
      alert(
        (adminLocale === "ko" ? "삭제 실패: " : "Delete failed: ") + e.message,
      );
    }
  };
    ...
    try {
      ...
      if (fetchError) {
        ...
        alert(
          (adminLocale === "ko"
            ? "어뷰징 데이터 조회 실패: "
            : "Abuse data load failed: ") + fetchError.message,
        );
        setAbuseData([]);
        return;
      }
      ...
      if (flaggedUsers && flaggedUsers.length > 0) {
        ...
        const { data: usersData } = await supabase
          .from("users")
          .select("user_id, name, nickname, status, suspension_reason")
          .in("user_id", customerIds);
        let emailMap: Record<string, string> = {};
        if (customerIds.length > 0) {
          const { data: emails } = await supabase.rpc("get_user_emails", {
            p_user_ids: customerIds,
          });
          if (emails)
            (emails as any[]).forEach((e) => {
              emailMap[e.user_id] = e.email;
        ...
        (usersData || []).forEach((u) => {
          userMap[u.user_id] = { ...u, email: emailMap[u.user_id] || "" };
        });
        setAbuseData(
          flaggedUsers.map((f: any) => ({
            ...f,
            userName:
              f.nickname ||
              f.name ||
              userMap[f.user_id]?.nickname ||
              userMap[f.user_id]?.name ||
              f.user_id.slice(0, 8),
  ...
  const handleUnflagConfirm = async () => {
    ...
    if (error) {
      showToast(
        (adminLocale === "ko" ? "해제 실패: " : "Unflag failed: ") +
          error.message,
        "error",
      );
    } else {
  ...
  const loadSearchLogs = useCallback(async () => {
    ...
    const { data, error } = await supabase.rpc("get_search_fail_stats", {
      limit_val: 50,
    });
  ...
  const loadAuditLogs = async (page = 1, filter = "all") => {
    ...
    if (filter === "ADMIN_ROLE_CHANGE") {
      q = q.in("action_type", ["PROMOTE_ADMIN", "REVOKE_ADMIN"]);
    } else if (filter !== "all") {
      q = q.eq("action_type", filter);
    }
  ...
  const fetchChatPreview = async (reportId: string, roomId: string) => {
    const { data } = await supabase
      .from("chat_messages")
      .select(
        "content, sender_id, created_at, message_type, sender:sender_id(name, nickname)",
      )
      .eq("room_id", roomId)
      .order("created_at", { ascending: false })
      .limit(30);
  ...
  const handleSuspend = async () => {
    ...
    const activeReport = reports.find(
      (r) =>
        r.reported?.user_id === reportSuspendModal.userId &&
        r.status === "pending",
    );
    const reportUserName = activeReport
      ? activeReport.reported?.nickname ||
        activeReport.reported?.name ||
    ...
    if (userError) {
      showToast(
        (adminLocale === "ko" ? "제재 처리 실패: " : "Sanction failed: ") +
          userError.message,
        "error",
      );
    ...
    if (activeReportId) {
      await supabase
        ...
        .update({
          status: "reviewed",
          admin_note: reportSuspendReason.trim(),
    ...
    const targetReport = reports.find(
      (r) => r.reported?.user_id === reportSuspendModal?.userId,
    );
    ...
    if (targetRoomId) {
      ...
      console.log(
        "[handleSuspend] chat_rooms CLOSED result:",
        roomError ? roomError.message : "success",
      );
    }
    ...
    await supabase.from("notifications").insert({
      ...
      message:
        adminLocale === "ko"
          ? `[관리자] 귀하의 계정에 ${suspendLabel} 처리가 되었습니다. 사유: ${reportSuspendReason}`
          : `[Admin] Your account has been ${suspendLabel}. Reason: ${reportSuspendReason}`,
      ...
      const systemMsg =
        suspendType === "warning"
          ? adminLocale === "ko"
            ...
            : `🚨 [Admin] A warning has been issued to your account.\nReason: ${reportSuspendReason}\nRepeated violations may result in suspension.`
          : suspendType === "temporary"
            ? adminLocale === "ko"
              ? `🚫 [관리자] 귀하의 계정이 ${suspendDays}일간 임시정지 처리되었습니다.\n사유: ${reportSuspendReason}`
              : `🚫 [Admin] Your account has been suspended for ${suspendDays} days.\nReason: ${reportSuspendReason}`
            : adminLocale === "ko"
              ? `🚫 [관리자] 귀하의 계정이 영구정지 처리되었습니다.\n사유: ${reportSuspendReason}`
              : `🚫 [Admin] Your account has been permanently banned.\nReason: ${reportSuspendReason}`;
    ...
    if (activeReport?.reporter?.user_id) {
      await supabase.from("notifications").insert({
        ...
        message:
          adminLocale === "ko"
            ? `✅ 신고하신 내용이 검토되어 처리 완료되었습니다.`
            : `✅ Your report has been reviewed and processed.`,
        reference_id: targetRoomId || null,
        is_read: false,
      });
    }
    ...
    showToast(
      adminLocale === "ko"
        ? `✅ ${reportUserName}님 제재 완료`
        : `✅ ${reportUserName} sanctioned`,
      "success",
      true,
    );
    fetchReports();
  ...
  useEffect(() => {
    if (!authorized) return;
    // ─── 읽음 마커 upsert ───
    const markerKeys: Record<string, string> = {
      inquiries: "inquiries",
      abuse: "abuse",
      reports: "reports",
      payout: "payout",
    ...
    if (markerKeys[tab]) {
      supabase
        .rpc("upsert_admin_read_marker", { p_marker_key: markerKeys[tab] })
        .then(() => {
    ...
    if (tab === "ledger") {
      setLedgerPage(1);
      loadLedger(1, ledgerCategory, ledgerPeriod, ledgerSearch);
    }
    if (tab === "pro") loadPros();
    ...
    (async () => {
      try {
        ...
        const { data: msgs } = await supabase
          .from("chat_messages")
          .select(
            "message_id, content, created_at, sender_id, sender:users!sender_id(name, nickname)",
          )
  ...
  const handleMapSearchTag = async (keyword: string) => {
    ...
    if (!category)
      return alert(
        adminLocale === "ko"
          ? "매핑할 카테고리를 먼저 선택해주세요."
          : "Please select a category to map first.",
      );
    if (
      !window.confirm(
        adminLocale === "ko"
          ? `'${keyword}' 단어를 '${category}' 카테고리의 동의어(search_tags)에 추가하고 로그에서 즉시 삭제하시겠습니까?`
          : `Add '${keyword}' as a synonym for '${category}' and delete from logs?`,
      )
    )
      return;
    ...
    if (error)
      return alert(
        (adminLocale === "ko" ? "매핑 실패: " : "Mapping failed: ") +
          error.message,
      );
    ...
    alert(
      adminLocale === "ko"
        ? "매핑 및 로그 삭제가 완료되었습니다."
        : "Mapping and log deletion complete.",
    );
  };
  ...
  const handleIgnoreSearchKeyword = async (keyword: string) => {
    if (
      !window.confirm(
        adminLocale === "ko"
          ? `'${keyword}' 로그를 영구 삭제(무시)하시겠습니까?`
          : `Permanently delete '${keyword}' log?`,
      )
    )
      return;
    ...
    if (error)
      return alert(
        (adminLocale === "ko" ? "삭제 실패: " : "Delete failed: ") +
          error.message,
      );
  ...
  const handleReplyImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    selected.forEach((file) => {
      ...
      reader.onload = (ev) =>
        setReplyImagePreviews((prev) => [...prev, ev.target?.result as string]);
  ...
  const handleSaveInquiryReply = async () => {
    ...
    await supabase.from("notifications").insert({
      ...
      message:
        adminLocale === "ko"
          ? `✅ 1:1 문의 답변이 등록되었습니다. [${selectedInquiry.title}]`
          : `✅ Your inquiry has been answered. [${selectedInquiry.title}]`,
  ...
  const handleUpdateInquiryStatus = async (
    ...
    showToast(
      adminLocale === "ko" ? "상태가 변경되었습니다." : "Status changed.",
      "success",
    );
    setInquiryStatusUpdating(false);
  };
  ...
  const handleSaveSupportPage = async (e: React.FormEvent) => {
    ...
    if (error) {
      alert(
        (adminLocale === "ko" ? "저장 중 오류 발생: " : "Error saving: ") +
          error.message,
      );
    } else {
      alert(
      ...
      setSupportPages((prev) =>
        prev.map((p) =>
          p.id === editingSupportPage.id ? editingSupportPage : p,
        ),
      );
  ...
  const executeSuspendToggle = async (
    ...
    if (error) {
      setModal({
        type: "error",
        title: adminLocale === "ko" ? "처리 실패" : "Action Failed",
    ...
    setModal({
      ...
      message:
        adminLocale === "ko"
          ? `계정이 ${next === "SUSPENDED" ? "정지" : "활성화"} 되었습니다.`
          : `Account has been ${next === "SUSPENDED" ? "suspended" : "activated"}.`,
    });

    // 목록 상태 업데이트
    if (isPro) {
      setPros((prev) =>
        prev.map((p) => (p.pro_id === userId ? { ...p, status: next } : p)),
      );
    } else {
      setCustomers((prev) =>
        prev.map((c) => (c.user_id === userId ? { ...c, status: next } : c)),
      );
    }
  };

  const handleCashAction = async () => {
    if (!cashModal || !cashAmount || Number(cashAmount) <= 0) {
      setModal({
        type: "error",
        title: adminLocale === "ko" ? "입력 오류" : "Input Error",
        message:
    ...
    if (rpcError) {
      setModal({
        type: "error",
        title: adminLocale === "ko" ? "트랜잭션 실패" : "Transaction Failed",
        message:
          (adminLocale === "ko" ? "전체 롤백됨: " : "Rolled back: ") +
          rpcError.message,
      });
    ...
    const newBal =
      ...
      pro.current_cash + (type === "charge" ? amt : -amt);
    setPros((p) =>
      p.map((x) =>
        x.pro_id === pro.pro_id ? { ...x, current_cash: newBal } : x,
      ),
    );
    ...
    const { error: logErr } = await supabase.from("admin_action_logs").insert({
      ...
      reason:
        adminLocale === "ko"
          ? `${cashType === "BONUS" ? "[보너스] " : "[유상] "}₱${amt.toLocaleString()} ${type === "charge" ? "충전" : "환불/차감"} — ${cashDesc || "사유 없음"}`
          : `${cashType === "BONUS" ? "[Bonus] " : "[Paid] "}${amt.toLocaleString()} credits ${type === "charge" ? "Top-up" : "Refund/Deduction"} — ${cashDesc || "No reason"}`,
    });
    ...
    setModal({
      type: "success",
      title: adminLocale === "ko" ? "처리 완료" : "Complete",
      message:
        adminLocale === "ko"
          ? `${cashType === "BONUS" ? "🎁 보너스 " : ""}₱${fmtNum(amt)} ${type === "charge" ? "충전" : "환불/차감"} 완료 (원자적 트랜잭션 + 원장 기록 완료)`
          : `${cashType === "BONUS" ? "🎁 Bonus " : ""}${fmtNum(amt)} credits ${type === "charge" ? "top-up" : "refund/deduction"} complete (atomic transaction + ledger recorded)`,
    });
  };
  ...
  const toggleReviewFeatured = async (
    ...
    if (
      !window.confirm(
        adminLocale === "ko"
          ? `이 리뷰를 ${action} 처리하겠습니까?\n(실제 메인에는 평점 4.5 이상이고 길이가 50자 이상인 경우에만 노출됩니다.)`
          : `${action} this review?\n(Only reviews with 4.5+ rating and 50+ characters are shown on main.)`,
      )
    )
      return;
    await supabase
      .from("reviews")
      .update({ is_featured_on_main: !curFeatured })
      .eq("review_id", reviewId);
    setReviews((r) =>
      r.map((x) =>
        x.review_id === reviewId
          ? { ...x, is_featured_on_main: !curFeatured }
          : x,
      ),
    );
    // ── 감사 로그 기록 ──
  ...
  const toggleReviewHidden = async (reviewId: number, curHidden: boolean) => {
    ...
    await supabase
      .from("reviews")
      .update({ is_hidden: !curHidden })
      .eq("review_id", reviewId);
    setReviews((r) =>
      r.map((x) =>
        x.review_id === reviewId ? { ...x, is_hidden: !curHidden } : x,
      ),
    );
    // ── 감사 로그 기록 ──
    const { error: logErr } = await supabase.from("admin_action_logs").insert({
  ...
  const deleteReview = async (reviewId: number) => {
    if (
      !window.confirm(
        adminLocale === "ko"
          ? "이 리뷰를 영구 삭제하겠습니까? 이 작업은 되돌릴 수 없습니다."
  ...
  const handleSelectRequest = async (r: any) => {
    ...
    const pData = proId
      ? (usersData || []).find((u) => u.user_id === proId)
      : null;
    ...
    setCsContactInfo({
      customer: cData || {
        nickname: r.customerName,
        email: adminLocale === "ko" ? "가져오기 실패" : "Load failed",
      },
      pro: proId
        ? {
            ...(pData || {}),
  ...
  const handleOpenProDetail = async (proId: string) => {
    ...
    try {
      ...
      const { data: uData } = await supabase
        .from("users")
        .select(
          "user_id, name, nickname, phone, status, created_at, avatar_url",
        )
        .eq("user_id", proId)
        .single();
      ...
      const pData =
        (allProfiles || []).find((p: any) => p.pro_id === proId) || null;
  ...
  const handleProDetailTabChange = async (
    newTab: "info" | "ledger" | "quotes" | "reviews",
    proId: string,
  ) => {
    ...
    try {
      if (newTab === "ledger") {
        ...
        setProDetailTabData((prev) => ({
          ...prev,
          ledger: (data || []).slice(0, PAGE),
        }));
  ...
  const handleProDetailLedgerMore = async (proId: string) => {
    ...
    try {
      ...
      setProDetailTabData((prev) => ({
        ...prev,
        ledger: [...(prev.ledger || []), ...fetched.slice(0, PAGE)],
      }));
  ...
  const handleForceCancelMatch = async () => {
    ...
    if (error) {
      alert(
        (adminLocale === "ko" ? "강제 취소 실패: " : "Force cancel failed: ") +
          error.message,
      );
    } else {
      alert(
  ...
  const handleSuspendUserWithReason = async () => {
    ...
    const nextStatus =
      suspendModal.currentStatus === "SUSPENDED" ? "ACTIVE" : "SUSPENDED";

    if (
      nextStatus === "SUSPENDED" &&
      (!suspendReason || suspendReason.trim().length < 5)
    ) {
      alert(
        adminLocale === "ko"
          ? "정지 사유를 5자 이상 입력해주세요 (감사 로그용)."
          : "Please enter a reason (min 5 chars) for the audit log.",
      );
      return;
    }
    ...
    if (error) {
      alert(
        (adminLocale === "ko"
          ? "유저 상태 변경 실패: "
          : "User status change failed: ") + error.message,
      );
    } else {
      ...
      if (logErr) {
        ...
        alert(
          (adminLocale === "ko"
            ? "⚠️ 정지 처리는 성공했으나 이력 기록 실패: "
            : "⚠️ Suspension succeeded but audit log failed: ") +
            logErr.message,
        );
      }
      ...
      alert(
        adminLocale === "ko"
          ? `계정이 ${nextStatus === "SUSPENDED" ? "정지" : "활성화"} 되었습니다.`
          : `Account has been ${nextStatus === "SUSPENDED" ? "suspended" : "activated"}.`,
      );
      // View Update
      if (suspendModal.role === "PRO") {
        setPros((p) =>
          p.map((x) =>
            x.pro_id === suspendModal.userId ? { ...x, status: nextStatus } : x,
          ),
        );
      } else if (suspendModal.role === "CUSTOMER") {
        setCustomers((c) =>
          c.map((x) =>
            x.user_id === suspendModal.userId
              ? { ...x, status: nextStatus }
              : x,
          ),
        );
      ...
      setAbuseData((prev) =>
        prev.map((a) =>
          a.user_id === suspendModal.userId
            ? {
                ...a,
                userStatus: nextStatus,
                suspensionReason:
                  nextStatus === "SUSPENDED" ? suspendReason : "",
              }
            : a,
        ),
      );
  ...
  const handleDeleteUser = async (userId: string, role: "PRO" | "CUSTOMER") => {
    if (
      !window.confirm(
        adminLocale === "ko"
          ? "⚠️ 이 사용자를 삭제 처리하시겠습니까?\n(소프트 삭제: 데이터는 보존되며, status가 DELETED로 변경됩니다)"
          : "⚠️ Delete this user?\n(Soft delete: data is preserved, status changes to DELETED)",
      )
    )
      return;
    const deleteReason = window.prompt(
      adminLocale === "ko"
        ? "삭제 사유를 입력하세요 (5자 이상, 감사 로그용):"
        : "Enter delete reason (min 5 chars, for audit log):",
    );
    if (!deleteReason || deleteReason.trim().length < 5) {
      setModal({
        ...
        message:
          adminLocale === "ko"
            ? "삭제 사유를 5자 이상 입력해야 합니다."
            : "Delete reason must be at least 5 characters.",
      });
      return;
    }
    ...
    if (error) {
      setModal({
        type: "error",
        title: adminLocale === "ko" ? "삭제 실패" : "Delete Failed",
        message:
          (adminLocale === "ko" ? "삭제 처리 실패: " : "Delete failed: ") +
          error.message,
      });
      return;
    }
    ...
    if (role === "PRO") {
      await supabase
        .from("pro_profiles")
        .update({ is_accepting_requests: false })
        .eq("pro_id", userId);
      setPros((p) =>
        p.map((x) => (x.pro_id === userId ? { ...x, status: "DELETED" } : x)),
      );
    } else {
      setCustomers((c) =>
        c.map((x) => (x.user_id === userId ? { ...x, status: "DELETED" } : x)),
      );
    }
    ...
    if (logErr) {
      ...
      setModal({
        type: "error",
        title: adminLocale === "ko" ? "이력 기록 실패" : "Audit Log Failed",
        message:
          (adminLocale === "ko"
            ? "삭제 처리는 성공했으나 이력 기록 실패: "
            : "Delete succeeded but audit log failed: ") + logErr.message,
      });
      return;
    }
    ...
    setModal({
      type: "success",
      title: adminLocale === "ko" ? "삭제 완료" : "Deleted",
      message:
        adminLocale === "ko"
          ? "계정이 삭제 처리되었습니다."
  ...
  const filteredPros = pros.filter((p) => {
    ...
    const match =
      !q ||
      [p.name, p.nickname, p.pro_id, p.email].some((v) =>
        (v || "").toLowerCase().includes(q),
      );
    const filt =
      proFilter === "all" ||
      (proFilter === "suspended" && p.status === "SUSPENDED") ||
      (proFilter === "deleted" && p.status === "DELETED");
    return match && filt;
  });
  ...
  const filteredCustomers = customers.filter((c) => {
    ...
    return (
      !q ||
      [c.name, c.nickname, c.user_id, c.email, c.phone].some((v) =>
        (v || "").toLowerCase().includes(q),
      )
    );
  });
  ...
  const filteredReviews = reviews.filter((r) => {
    ...
    return (
      !q ||
      [r.customerName, r.proName, r.comment].some((v) =>
        (v || "").toLowerCase().includes(q),
      )
    );
  });
  ...
  if (!authorized || loading)
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center text-gray-400 text-lg">
        {adminLocale === "ko" ? "인증 확인 중..." : "Authenticating..."}
      </div>
    );
  ...
  const menuGroup1: { key: AdminTab; icon: string; label: string }[] = [
    {
      key: "dashboard",
      icon: "📊",
      label: adminLocale === "ko" ? "대시보드" : "Dashboard",
    },
    {
      key: "search_logs",
      icon: "🔍",
      label: adminLocale === "ko" ? "검색/유입 분석" : "Search Analytics",
    },
  ];
  ...
  const userSubMenuItems: { key: AdminTab; icon: string; label: string }[] = [
    ...(adminRole === "ADMIN" || adminRole === "ADMIN_OPERATION"
      ? [
          {
            key: "customer" as AdminTab,
            icon: "👥",
            label: adminLocale === "ko" ? "고객 관리" : "Customers",
          },
          {
            key: "pro" as AdminTab,
            icon: "🔧",
            label: adminLocale === "ko" ? "고수 관리" : "Pros",
          },
        ]
      : []),
    ...(adminRole === "ADMIN"
      ? [
          {
            key: "admin_mgmt" as AdminTab,
            icon: "🛡️",
            label: adminLocale === "ko" ? "관리자 관리" : "Admin Mgmt",
          },
        ]
      : []),
  ];
  ...
  const menuGroup3: { key: AdminTab; icon: string; label: string }[] = [
    {
      key: "ledger",
      icon: "💰",
      label: adminLocale === "ko" ? "캐시 원장" : "Credit Ledger",
    },
    {
      key: "payout",
      icon: "💸",
      label: adminLocale === "ko" ? "출금 관리" : "Payouts",
    },
  ];
  // 📋 서비스 매칭 운영
  const menuGroup4: { key: AdminTab; icon: string; label: string }[] = [
    {
      key: "quotes",
      icon: "📋",
      label: adminLocale === "ko" ? "견적/매칭 내역" : "Quotes & Matches",
    },
    {
      key: "reviews",
      icon: "⭐",
      label: adminLocale === "ko" ? "리뷰 관리" : "Reviews",
    },
    {
      key: "referral",
      icon: "🎁",
      label: adminLocale === "ko" ? "추천인 관리" : "Referral Mgmt",
    },
  ];
  ...
  const menuGroup5: { key: AdminTab; icon: string; label: string }[] = [
    {
      key: "inquiries",
      icon: "💬",
      label: adminLocale === "ko" ? "1:1 문의 관리" : "Inquiries",
    },
    {
      key: "abuse",
      icon: "⚠️",
      label: adminLocale === "ko" ? "어뷰징 관리" : "Abuse Mgmt",
    },
  ];
  // ⚙️ 시스템 및 환경 설정
  const menuGroup6: { key: AdminTab; icon: string; label: string }[] = [
    {
      ...
      icon: "🏷️",
      label:
        adminLocale === "ko" ? "카테고리/단가표 설정" : "Categories & Pricing",
    },
    {
      key: "cms",
      icon: "🖼️",
      label: adminLocale === "ko" ? "CMS (홈/접객 관리)" : "CMS (Home Mgmt)",
    },
    {
      key: "audit_log",
      icon: "🔒",
      label: adminLocale === "ko" ? "감사 로그" : "Audit Log",
    },
    {
      key: "settings",
      icon: "⚙️",
      label: adminLocale === "ko" ? "환경 설정" : "Settings",
    },
  ];
  ...
  const isUserTab = (key: AdminTab) =>
    ["pro", "customer", "admin_mgmt"].includes(key);
  ...
  const filteredReports = reports
    ...
    .filter((r) => {
      ...
      return (
        (r.reporter?.nickname || r.reporter?.name || "")
          .toLowerCase()
          .includes(q) ||
        (r.reported?.nickname || r.reported?.name || "")
          .toLowerCase()
          .includes(q) ||
    ...
    .sort((a, b) => {
      if (reportSort === "most_reported")
        return (b.reportedTotalCount ?? 0) - (a.reportedTotalCount ?? 0);
      return (
  ...
  return (
    <div className="flex min-h-screen bg-gray-900 text-white">
      {/* ─── Sidebar ─── */}
      <aside
        className={`${sidebarOpen ? "w-60" : "w-16"} bg-gray-950 border-r border-gray-800 flex flex-col shrink-0 transition-all duration-200`}
      >
        ...
        <nav className="flex-1 p-2 overflow-y-auto">
            ...
            >
              ...
              {sidebarOpen && (
                <>
                  ...
                  <span
                    className={`text-xs transition-transform ${group1Open ? "rotate-180" : ""}`}
                  >
                    ▾
                  </span>
                </>
              ...
              <div className="space-y-0.5">
                ...
                >
                  ...
                  {sidebarOpen && (
                    <>
                      <span className="flex-1 text-left">
                        {adminLocale === "ko" ? "사용자 관리" : "User Mgmt"}
                      </span>
                      <span
              ...
              >
                ...
                {sidebarOpen && (
                  <>
                    ...
                    <span
                      className={`text-xs transition-transform ${group3Open ? "rotate-180" : ""}`}
                    >
                      ▾
                    </span>
                  </>
                ...
                <div className="ml-4 border-l border-gray-700 pl-2 space-y-0.5">
                  {menuGroup3.map((m) => (
                    ...
                    >
                      ...
                      {m.label}
                      {m.key === "payout" && (
                        <UnreadBadge count={unreadCounts.payout} />
                      )}
                    </button>
                  ))}
                </div>
              ...
              >
                ...
                {sidebarOpen && (
                  <>
                    ...
                    <span
                      className={`text-xs transition-transform ${group4Open ? "rotate-180" : ""}`}
                    >
                      ▾
                    </span>
                  </>
            ...
            <div className="space-y-0.5">
              ...
              >
                ...
                {sidebarOpen && (
                  <>
                    <span className="flex-1 text-left">
                      {adminLocale === "ko" ? "CS 및 리스크 관리" : "CS & Risk"}
                    </span>
                    <span
                      className={`text-xs transition-transform ${group5Open ? "rotate-180" : ""}`}
                    >
                      ▾
                    </span>
                  </>
                ...
                <div className="ml-4 border-l border-gray-700 pl-2 space-y-0.5">
                  {menuGroup5.map((m) => (
                    ...
                    >
                      ...
                      {m.label}
                      {m.key === "inquiries" && (
                        <UnreadBadge count={unreadCounts.inquiries} />
                      )}
                      {m.key === "abuse" && (
                        <UnreadBadge count={unreadCounts.abuse} />
                      )}
                    </button>
                  ))}
                  {/* 신고 관리 — 미처리 건수 배지 유지 */}
              ...
              >
                ...
                {sidebarOpen && (
                  <>
                    ...
                    <span
                      className={`text-xs transition-transform ${group6Open ? "rotate-180" : ""}`}
                    >
                      ▾
                    </span>
                  </>
          ...
          >
            <span>🚪</span>
            {sidebarOpen && (
              <span>{adminLocale === "ko" ? "로그아웃" : "Logout"}</span>
            )}
          </button>
        </nav>
      </aside>
        ...
        <div className="max-w-7xl mx-auto p-6">
          ...
          {userDetail ? (
            <div>
              ...
              >
                {adminLocale === "ko" ? "← 목록으로" : "← Back to list"}
              </button>
                  ...
                  <div className="w-14 h-14 bg-gray-700 rounded-full flex items-center justify-center text-2xl overflow-hidden flex-shrink-0">
                    {detailData.user?.avatar_url ? (
                      <img
                        ...
                        alt={adminLocale === "ko" ? "프로필" : "Profile"}
                  ...
                  <div>
                    <h2 className="text-xl font-bold">
                      {detailData.user?.nickname ||
                        ...
                        (adminLocale === "ko" ? "사용자" : "User")}
                    </h2>
                    <p className="text-xs text-gray-500">
                      {userDetail.role} · {detailData.user?.status || "ACTIVE"}
                    </p>
                  </div>
                    ...
                    <div className="ml-auto text-right">
                      ...
                      {(detailData.profile.bonus_cash || 0) > 0 && (
                        <p className="text-sm font-bold text-green-400">
                          +🎁{fmtNum(detailData.profile.bonus_cash)}{" "}
                          {adminLocale === "ko" ? "보너스" : "Bonus"}
                        </p>
                      )}
                      <p className="text-xs text-gray-500">
                        {adminLocale === "ko" ? "캐시 잔액" : "Credit Balance"}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex gap-1 mb-4 bg-gray-800/50 rounded-xl p-1">
                {(
                  ["info", "ledger", "quotes", "reviews"] as UserDetailTab[]
                ).map((t) => {
                  const isDisabled =
                    t === "ledger" && userDetail.role === "CUSTOMER";
                  return (
                    <button
                      key={t}
                      onClick={() => !isDisabled && setUserDetailTab(t)}
                    ...
                    <div>
                      ...
                      <span className="text-white">
                        {detailData.user?.created_at
                          ? fmtDate(detailData.user.created_at)
                          : "-"}
                      </span>
                    </div>
                    {userDetail.role === "PRO" && detailData.profile && (
                      <>
                        ...
                        <div>
                          <span className="text-gray-500 block text-xs mb-1">
                            {adminLocale === "ko" ? "서비스" : "Services"}
                          </span>
                          <span className="text-white">
                            {(detailData.profile.services || []).join(", ") ||
                              "-"}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-500 block text-xs mb-1">
                            {adminLocale === "ko" ? "지역" : "Region"}
                          </span>
                          <span className="text-white">
                            {detailData.profile.region || "-"}
                          </span>
                        </div>
                        <div>
                          <span className="text-gray-500 block text-xs mb-1">
                          ...
                          <span className="text-white">
                            {detailData.profile.facebook_url || "-"}
                          </span>
                        </div>
                      </>
                    )}
                    ...
                    <thead>
                      <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase">
                        <th className="p-2 text-left">
                          {adminLocale === "ko" ? "시간" : "Time"}
                        </th>
                        <th className="p-2 text-left">
                          {adminLocale === "ko" ? "유형" : "Type"}
                        </th>
                        <th className="p-2 text-right">
                          {adminLocale === "ko" ? "금액" : "Amount"}
                        </th>
                        <th className="p-2 text-right">
                          {adminLocale === "ko" ? "잔액" : "Balance"}
                        </th>
                        <th className="p-2 text-left">
                          {adminLocale === "ko" ? "설명" : "Desc"}
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {(detailData.ledger || []).map((tx: any, i: number) => (
                        <tr key={i} className="border-b border-gray-700/50">
                          ...
                          <td className="p-2 text-right text-gray-400 text-xs">
                            {fmtNum(Number(tx.balance_snapshot))}
                          </td>
                          <td className="p-2 text-gray-500 text-xs">
                            {txDesc(tx.tx_type, tx.description)}
                          </td>
                        </tr>
                      ))}
                      {(detailData.ledger || []).length === 0 && (
                        <tr>
                          ...
                          >
                            {adminLocale === "ko" ? "내역 없음" : "No records"}
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                {userDetailTab === "quotes" && (
                  <div className="space-y-2">
                    {userDetail.role === "CUSTOMER"
                      ? (detailData.requests || []).map((r: any) => (
                          <div
                            key={r.request_id}
                            className="p-3 bg-gray-700/30 rounded-xl"
                          >
                            <div className="flex justify-between items-center">
                              <span className="font-semibold text-sm">
                                {r.service_type ||
                                  (adminLocale === "ko" ? "서비스" : "Service")}
                              </span>
                              <span
                                className={`text-xs font-bold px-2 py-0.5 rounded-full ${statusColor(r.status)}`}
                              >
                                {statusLabel(r.status)}
                              </span>
                            </div>
                            <p className="text-xs text-gray-400 mt-1">
                              {r.region} · {fmtDate(r.created_at)} ·{" "}
                              {adminLocale === "ko"
                                ? `견적 ${(r.match_quotes || []).length}건`
                                : `${(r.match_quotes || []).length} quotes`}
                            </p>
                          </div>
                        ))
                      : (detailData.quotes || []).map((q: any) => (
                          <div
                            key={q.quote_id}
                            className="p-3 bg-gray-700/30 rounded-xl flex justify-between items-center"
                          >
                            <div>
                ...
                {userDetailTab === "reviews" && (
                  <div className="space-y-2">
                    ...
                    {(detailData.reviews || []).length === 0 && (
                      <p className="text-center text-gray-500 py-6">
                        {adminLocale === "ko" ? "리뷰 없음" : "No reviews"}
                      </p>
                    )}
                  </div>
                )}
            ...
            <>
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
                    ...
                    >
                      ...
                      <span
                        className={`text-2xl font-black ${pendingInquiries > 0 ? "text-orange-400" : "text-white"}`}
                      >
                        {pendingInquiries}
                      </span>
                    </div>
                    ...
                    {[
                      ...
                      },
                      {
                        ...
                        onClick: () => {
                          ...
                          handleTabClick("payout");
                        },
                      },
                      {
                        l: adminLocale === "ko" ? "👥 고객" : "👥 Customers",
                        v: stats.customers,
                        onClick: () => handleTabClick("customer"),
                      },
                      {
                        l: adminLocale === "ko" ? "🔧 고수" : "🔧 Pros",
                        v: stats.pros,
                      ...
                      },
                      {
                        ...
                        p: "-",
                        onClick: () => {
                          setLedgerCategory("DEDUCT_QUOTE");
                          setLedgerPeriod("today");
                          setLedgerPage(1);
                          handleTabClick("ledger");
                        },
                      ...
                      {
                        l: adminLocale === "ko" ? "⚠️ 어뷰징" : "⚠️ Abuse",
                        v: stats.abuseCount,
                        c: "text-orange-400",
                        onClick: () => handleTabClick("abuse"),
                      },
                      {
                        l:
                          adminLocale === "ko"
                      ...
                      >
                        ...
                        <p
                          className={`text-2xl font-black ${s.c || "text-white"}`}
                        >
                          {s.p || ""}
                          {fmtNum(s.v)}
                        </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                    <div className="bg-gradient-to-br from-purple-900/40 to-purple-800/20 rounded-xl p-4 border border-purple-700/40">
                      <p className="text-purple-300 text-xs font-bold uppercase mb-1">
                        {adminLocale === "ko"
                          ? "🏦 총 부채 (전체 고수 잔액)"
                          : "🏦 Total Liability (All Pro Balances)"}
                      </p>
                    ...
                    <div className="bg-gradient-to-br from-red-900/40 to-red-800/20 rounded-xl p-4 border border-red-700/40">
                      <p className="text-red-300 text-xs font-bold uppercase mb-1">
                        {adminLocale === "ko"
                          ? "📉 유출 합계 (차감/환불)"
                          : "📉 Total Out (Deduct/Refund)"}
                      </p>
                      <p className="text-2xl font-black text-red-400">
                        -{fmtNum(ledgerStats.totalOut)}
                      </p>
                    </div>
                    <div className="bg-gradient-to-br from-gray-800 to-gray-700/30 rounded-xl p-4 border border-gray-600/40">
                      <p className="text-gray-300 text-xs font-bold uppercase mb-1">
                        {adminLocale === "ko"
                          ? "🔢 조회 건수"
                          : "🔢 Result Count"}
                      </p>
                      <p className="text-2xl font-black text-white">
                        {fmtNum(ledgerStats.filtered)}
                        {adminLocale === "ko" ? "건" : ""}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mb-4">
                    <div className="flex gap-2 flex-1">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            setLedgerSearch(ledgerSearchInput);
                            setLedgerPage(1);
                            loadLedger(
                              1,
                              ledgerCategory,
                    ...
                    <select
                      value={ledgerPeriod}
                      onChange={(e) => {
                        const v = e.target.value as LedgerPeriod;
                        setLedgerPeriod(v);
                        setLedgerPage(1);
                        loadLedger(1, ledgerCategory, v, ledgerSearch);
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        const csvRows = ledgerData
                          ...
                          .join("\n");
                        const bom = "\uFEFF";
                        const blob = new Blob([bom + header + csvRows], {
                      ...
                      className="bg-green-600 hover:bg-green-700 text-white px-4 py-2.5 rounded-lg text-sm font-bold whitespace-nowrap"
                    >
                      {adminLocale === "ko"
                        ? "📥 CSV 다운로드"
                        : "📥 Download CSV"}
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase">
                            ...
                            <th className="p-2.5 text-left">
                              {adminLocale === "ko" ? "고수명" : "Pro Name"}
                            </th>
                            <th className="p-2.5 text-left">
                              {adminLocale === "ko" ? "이메일" : "Email"}
                            </th>
                            <th className="p-2.5 text-left">
                              {adminLocale === "ko" ? "전화번호" : "Phone"}
                            </th>
                        ...
                        <tbody>
                          {ledgerData.map((tx) => (
                            ...
                            >
                              <td className="p-2.5 text-gray-400 text-xs whitespace-nowrap">
                                {fmtDate(tx.created_at)}
                              </td>
                                ...
                                <button
                                  onClick={async () => {
                                    setDrilldown({
                                      ...
                                      proEmail: tx.proEmail || "",
                                      proPhone: tx.proPhone || "",
                                      txs: [],
                                      loading: true,
                                    });
                              ...
                              <td className="p-2.5 text-gray-500 text-xs max-w-[200px] truncate">
                                {txDesc(tx.tx_type, tx.description)}
                              </td>
                            </tr>
                          ))}
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <div className="flex gap-1">
                        <button
                          ...
                          className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-sm rounded-lg font-bold disabled:opacity-30"
                        >
                          {adminLocale === "ko" ? "← 이전" : "← Prev"}
                        </button>
                ...
                <>
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h1 className="text-2xl font-black text-white">
                        {adminLocale === "ko"
                          ? "⚠️ 어뷰징/패널티 관리"
                          : "⚠️ Abuse / Penalty Management"}
                      </h1>
                      <p className="text-sm text-gray-400 mt-1">
                        {adminLocale === "ko"
                          ? "3-15-0 룰 기반으로 자동 적발된 윈도우 쇼퍼 고객을 관리합니다."
                          : "Manage window-shopper customers auto-flagged by the 3-15-0 rule."}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="flex bg-gray-800 rounded-lg p-0.5">
                        <button
                          onClick={() => setAbuseFilter("flagged")}
                          className={`px-3 py-1.5 text-xs font-bold rounded-md transition ${abuseFilter === "flagged" ? "bg-red-600 text-white" : "text-gray-400 hover:text-white"}`}
                        >
                          {adminLocale === "ko"
                            ? "🚨 적발된 유저"
                            : "🚨 Flagged Users"}
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="bg-red-900/20 border border-red-800/30 rounded-xl p-5">
                      <p className="text-xs text-red-300/70 font-bold uppercase">
                        {adminLocale === "ko"
                          ? "총 적발 고객"
                          : "Total Flagged"}
                      </p>
                      <p className="text-3xl font-black text-red-400 mt-1">
                        {abuseData.filter((a) => a.is_flagged).length}
                      </p>
                    </div>
                    <div className="bg-orange-900/20 border border-orange-800/30 rounded-xl p-5">
                      <p className="text-xs text-orange-300/70 font-bold uppercase">
                        {adminLocale === "ko"
                          ? "최근 7일 신규 적발"
                          : "New Flags (7d)"}
                      </p>
                    ...
                    <div className="bg-green-900/20 border border-green-800/30 rounded-xl p-5">
                      <p className="text-xs text-green-300/70 font-bold uppercase">
                        {adminLocale === "ko" ? "해제 완료" : "Unflagged"}
                      </p>
                      <p className="text-3xl font-black text-green-400 mt-1">
                        {
                          abuseData.filter(
                            (a) => !a.is_flagged && a.total_noshow > 0,
                          ).length
                        }
                      </p>
                    </div>
                  ...
                  <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4 mb-6 text-sm text-gray-400">
                    <div className="flex items-center justify-between mb-2">
                      <p className="font-bold text-white">
                        {adminLocale === "ko"
                          ? "📋 3-15-0 자동 적발 조건 (AND)"
                          : "📋 3-15-0 Auto-Flag Conditions (AND)"}
                      </p>
                      <p className="text-xs text-orange-300 font-bold">
                        {adminLocale === "ko"
                          ? `전체 조건 달성: ${abuseData.filter((a) => a.is_abuse_target).length}명`
                          : `All conditions met: ${abuseData.filter((a) => a.is_abuse_target).length}`}
                      </p>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-gray-900/50 p-3 rounded-lg text-center">
                        ...
                        <p className="text-xs text-red-400/70 mt-1">
                          {adminLocale === "ko"
                            ? `${abuseData.filter((a) => a.consecutive_noshow >= 3).length}명 달성`
                            : `${abuseData.filter((a) => a.consecutive_noshow >= 3).length} reached`}
                        </p>
                      </div>
                      <div className="bg-gray-900/50 p-3 rounded-lg text-center">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {adminLocale === "ko"
                            ? "누적 열람 견적"
                            : "Cumulative Quotes Read"}
                        </p>
                        <p className="text-xs text-orange-400/70 mt-1">
                          {adminLocale === "ko"
                            ? `${abuseData.filter((a) => a.total_read_quotes >= 15).length}명 달성`
                            : `${abuseData.filter((a) => a.total_read_quotes >= 15).length} reached`}
                        </p>
                      </div>
                      <div className="bg-gray-900/50 p-3 rounded-lg text-center">
                        ...
                        <p className="text-xs text-gray-400/70 mt-1">
                          {adminLocale === "ko"
                            ? `${abuseData.filter((a) => a.match_rate === 0).length}명 달성`
                            : `${abuseData.filter((a) => a.match_rate === 0).length} reached`}
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase">
                            ...
                            <th className="p-4 text-center">
                              {adminLocale === "ko" ? "매칭률" : "Match Rate"}
                            </th>
                            <th className="p-4 text-center">
                              {adminLocale === "ko" ? "상태" : "Status"}
                            </th>
                            <th className="p-4 text-center">
                              {adminLocale === "ko" ? "적발일" : "Flagged At"}
                            </th>
                            <th className="p-4 text-center">
                              {adminLocale === "ko" ? "액션" : "Action"}
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {abuseData.length === 0 ? (
                            ...
                            abuseData.map((a: any) => {
                              const matchRateVal =
                                a.match_rate !== null &&
                                a.match_rate !== undefined
                                  ? a.match_rate
                                  : a.total_requests > 0
                                    ? Math.round(
                                        (a.total_matched / a.total_requests) *
                                          100,
                                      )
                                    : null;
                              return (
                                <tr
                                  key={a.user_id}
                                  className={`border-b border-gray-700/50 hover:bg-gray-700/20 ${!a.is_flagged ? "opacity-50" : ""}`}
                                >
                                  <td className="p-4">
                                    <p className="font-bold text-white">
                                      {a.nickname || a.name || a.userName}
                                    </p>
                                    <p className="text-xs text-gray-500">
                                      {a.userEmail}
                                    </p>
                                  ...
                                  <td
                                    className={`p-4 text-center font-bold ${a.consecutive_noshow >= 3 ? "text-red-400" : "text-gray-400"}`}
                                  >
                                    {a.consecutive_noshow}
                                    {adminLocale === "ko" ? "회" : "x"}
                                  </td>
                                  <td
                                    className={`p-4 text-center font-bold ${a.total_read_quotes >= 15 ? "text-orange-400" : "text-gray-400"}`}
                                  >
                                    {a.total_read_quotes}
                                    {adminLocale === "ko" ? "건" : ""}
                                  </td>
                                  <td
                                    className={`p-4 text-center font-bold ${matchRateVal === 0 ? "text-red-400" : "text-green-400"}`}
                                  >
                                    {matchRateVal !== null
                                      ? `${matchRateVal}%`
                                      : "-"}
                                  ...
                                  <td className="p-4 text-center">
                                    {a.is_flagged ? (
                                      <span className="bg-red-900/50 text-red-300 text-xs font-bold px-2 py-1 rounded-full">
                                        {adminLocale === "ko"
                                          ? "적발"
                                          : "Flagged"}
                                      </span>
                                    ) : (
                                      <span className="bg-green-900/50 text-green-300 text-xs font-bold px-2 py-1 rounded-full">
                                        {adminLocale === "ko"
                                          ? "해제"
                                          : "Cleared"}
                                      </span>
                                      ...
                                      <span className="relative group ml-1">
                                        ...
                                        {a.suspensionReason && (
                                          <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-white text-gray-900 text-xs rounded-lg px-3 py-2.5 whitespace-nowrap shadow-2xl z-50 max-w-xs border border-gray-200">
                                            <span className="block font-bold text-red-600 mb-1">
                                              {adminLocale === "ko"
                                                ? "📝 정지 사유"
                                                : "📝 Suspension Reason"}
                                            </span>
                                  ...
                                  <td className="p-4 text-center text-xs text-gray-500">
                                    {a.flagged_at ? fmtDate(a.flagged_at) : "-"}
                                  </td>
                                  <td className="p-4 text-center">
                    ...
                    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
                      <div className="bg-gray-900 rounded-xl p-6 w-full max-w-sm border border-gray-700 shadow-2xl">
                        ...
                        <p className="text-xs text-gray-500 mb-6">
                          {adminLocale === "ko"
                            ? "연속 노쇼 카운트도 0으로 초기화됩니다."
                            : "The consecutive no-show count will also be reset to 0."}
                        </p>
                  ...
                  {abuseTimeline && (
                    <div
                      ...
                      onClick={() => setAbuseTimeline(null)}
                    >
                      <div
                        className="bg-gray-900 rounded-2xl w-full max-w-lg max-h-[80vh] overflow-y-auto shadow-2xl border border-gray-700"
                      ...
                      >
                        <div className="sticky top-0 bg-gray-900 border-b border-gray-700 p-5 flex justify-between items-center z-10">
                          <div>
                            ...
                            <p className="text-sm text-gray-400 mt-1">
                              {abuseTimeline.user.userName} (
                              {abuseTimeline.user.userEmail})
                            </p>
                          </div>
                          <button
                            onClick={() => setAbuseTimeline(null)}
                          ...
                          <div className="flex gap-3">
                            <div className="bg-gradient-to-br from-red-900/50 to-red-800/30 border-2 border-red-500/60 rounded-xl px-4 py-3 flex-1 text-center shadow-[0_0_12px_rgba(239,68,68,0.2)]">
                              <p className="text-3xl font-black text-red-400 drop-shadow-lg">
                                {
                                  abuseTimeline.logs.filter(
                                    (l) => l.action_type === "SUSPEND",
                                  ).length
                                }
                              </p>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-gray-400 text-sm mt-1">
                        {adminLocale === "ko"
                          ? "채팅방에서 접수된 신고를 검토하고 제재를 처리합니다."
                          : "Review reports submitted from chat rooms and process sanctions."}
                      </p>
                    </div>
                    <button
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <input
                      ...
                      placeholder={
                        adminLocale === "ko"
                          ? "신고자/피신고자/사유 검색..."
                          : "Search reporter/reported/reason..."
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setReportSort(
                          e.target.value as "latest" | "most_reported",
                        )
                      }
                  ...
                  {reportsLoading ? (
                    ...
                    <div className="text-center py-10 text-gray-400">
                      {reports.length === 0
                        ...
                        : adminLocale === "ko"
                          ...
                          : "No reports match the search/filter."}
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {filteredReports.map((report) => (
                        <div
                          key={report.id}
                          className={`rounded-xl p-4 border ${report.status === "pending" ? "border-red-500/30 bg-red-500/5" : "border-white/10 bg-white/5"}`}
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-xs font-bold px-2 py-0.5 rounded-full ${report.status === "pending" ? "bg-red-500 text-white" : "bg-gray-600 text-gray-300"}`}
                              >
                                {adminLocale === "ko"
                            ...
                            >
                              {adminLocale === "ko"
                                ...
                                : `💬 Chat Log ${chatPreview?.reportId === report.id ? "Close" : "View"}`}
                            </button>
                          </div>
                              ...
                              <p className="text-red-400 font-medium flex items-center gap-1 flex-wrap">
                                ...
                                <span
                                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                                    report.reportedTotalCount >= 5
                                      ? "bg-red-500 text-white"
                                      : report.reportedTotalCount >= 3
                          ...
                          <div className="flex items-center justify-between">
                            {report.status === "pending" ? (
                              <button
                                onClick={() =>
                                  setReportSuspendModal({
                                    userId: report.reported?.user_id,
                                    role: (report.reported?.role?.toUpperCase() ||
                                      "") as "PRO" | "CUSTOMER" | "",
                                    currentStatus:
                              ...
                              <p className="text-xs text-gray-500">
                                {adminLocale === "ko"
                                  ? `처리 메모: ${report.admin_note}`
                                  : `Memo: ${report.admin_note}`}
                              </p>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                      <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 max-w-md w-full shadow-2xl">
                        ...
                        <div className="flex gap-2 mb-4">
                          {(["warning", "temporary", "permanent"] as const).map(
                            (type) => (
                              <button
                                key={type}
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setReportSuspendReason(e.target.value)
                          }
                          placeholder={
                            adminLocale === "ko"
                              ? "제재 사유를 입력하세요 (유저에게 표시됩니다)"
                              : "Enter sanction reason (shown to user)"
                          }
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            disabled={
                              suspendSubmitting || !reportSuspendReason.trim()
                            }
                ...
                <>
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h1 className="text-2xl font-black text-white">
                        {adminLocale === "ko"
                          ? "🏷️ 카테고리/단가표 관리"
                          : "🏷️ Category / Pricing Management"}
                      </h1>
                      <p className="text-sm text-gray-400 mt-1">
                        {adminLocale === "ko"
                          ? "플랫폼의 서비스 카테고리와 견적 발송 단가를 관리합니다."
                          : "Manage service categories and quote sending prices for the platform."}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        {Array.from(
                          new Set(
                            dbCategories.map((c) => c.depth1).filter(Boolean),
                          ),
                        )
                          .sort()
                          .map((d1: any) => (
                        ...
                        >
                          ...
                          {Array.from(
                            new Set(
                              dbCategories
                                .filter(
                                  (c) => c.depth1 === categoryDepth1Filter,
                                )
                                .map((c) => c.depth2)
                                .filter(Boolean),
                            ),
                  ...
                  {!editingCategory && !isAddingCategory ? (
                    <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-x-auto shadow-sm">
                      <table className="w-full text-sm text-left text-gray-400">
                        <thead className="text-xs uppercase bg-gray-900/50 text-gray-500">
                          <tr>
                            <th className="px-5 py-4 font-black text-gray-300">
                              Depth 1
                            </th>
                            <th className="px-5 py-4 font-black text-gray-300">
                              Depth 2
                            ...
                            <th className="px-5 py-4 font-black text-gray-300 text-right">
                              {adminLocale === "ko"
                                ? "기본 견적단가 (Cost)"
                                : "Base Quote Price"}
                            </th>
                            <th className="px-5 py-4 font-black text-gray-300 text-center">
                              {adminLocale === "ko" ? "활성 상태" : "Active"}
                            </th>
                            <th className="px-5 py-4 font-black text-gray-300 text-center">
                              {adminLocale === "ko" ? "수정일" : "Updated"}
                            </th>
                            <th className="px-5 py-4 text-right">
                              {adminLocale === "ko" ? "관리" : "Actions"}
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {dbCategories
                            .filter(
                              (cat) =>
                                (categoryDepth1Filter === "all" ||
                                  cat.depth1 === categoryDepth1Filter) &&
                                (categoryDepth2Filter === "all" ||
                                  cat.depth2 === categoryDepth2Filter),
                            )
                            .map((cat) => (
                              <tr
                                key={cat.id}
                              ...
                              >
                                ...
                                <td className="px-5 py-3 text-center">
                                  <span
                                    className={`px-2 py-1 rounded text-xs font-bold ${cat.is_active ? "bg-green-900/50 text-green-400" : "bg-red-900/50 text-red-500"}`}
                                  >
                                    {adminLocale === "ko"
                                      ? cat.is_active
                                        ? "운영 중"
                                ...
                                <td className="px-5 py-3 text-center text-gray-500 text-xs">
                                  {fmtDate(cat.updated_at || cat.created_at)}
                                </td>
                                <td className="px-5 py-3 text-right">
                                  ...
                                  <button
                                    ...
                                    className="text-red-500 hover:text-red-300 font-bold px-3 py-1 bg-red-900/20 rounded transition text-xs"
                                  >
                                    {adminLocale === "ko" ? "삭제" : "Delete"}
                                  </button>
                                </td>
                              </tr>
                            ))}
                          {dbCategories.filter(
                            ...
                            <tr>
                              ...
                              >
                                {adminLocale === "ko"
                                  ? "카테고리 데이터가 없습니다. DB 마이그레이션을 확인하세요."
                                  : "No category data. Check DB migration."}
                              </td>
                            </tr>
                          )}
                        </tbody>
                    ...
                    <form
                      ...
                      className="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-md max-w-2xl"
                    >
                      <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
                        ...
                        <button
                          ...
                          className="text-gray-400 hover:text-white font-bold p-2 bg-gray-700 rounded-full hover:bg-gray-600 transition"
                        >
                          ✕
                        </button>
                      </div>
                      <div className="space-y-6">
                        <div>
                          ...
                          />
                          {!isAddingCategory && (
                            <p className="text-xs text-orange-400 mt-2 font-bold py-1 px-2 bg-orange-900/20 rounded inline-block">
                              {adminLocale === "ko"
                                ? "🔗 기준 카테고리명 및 분류는 시스템 매칭 무결성을 위해 수정 불가합니다."
                                : "🔗 Category name and classification cannot be edited to preserve system integrity."}
                            </p>
                          )}
                        </div>
                        <div>
                          <label className="block text-sm font-bold text-gray-300 mb-2">
                            {adminLocale === "ko"
                              ? "기본 견적단가 (캐시 차감액)"
                              : "Base Quote Price (Credits Deducted)"}
                          </label>
                          <div className="relative">
                            <input
                              type="number"
                              min="0"
                              value={editingCategory.base_price}
                              onChange={(e) =>
                                setEditingCategory({
                                  ...editingCategory,
                      ...
                      <div className="mt-8 pt-6 border-t border-gray-700 flex justify-end gap-3">
                        <button
                          ...
                          className="px-5 py-2.5 rounded-lg text-sm font-bold text-gray-400 hover:text-white transition"
                        >
                          {adminLocale === "ko" ? "취소" : "Cancel"}
                        </button>
                        <button
                      ...
                      <div className="bg-gray-900 border border-red-900/50 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden shadow-red-900/20">
                        ...
                        <div className="p-5 flex gap-2 justify-end">
                          ...
                          <button
                            ...
                            className="px-5 py-2.5 rounded-lg text-sm font-bold bg-red-600 hover:bg-red-500 text-white transition"
                          >
                            {adminLocale === "ko"
                              ? "영구 삭제"
                              : "Delete Permanently"}
              ...
              {tab === "pro" && (
                <>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mb-4">
                    ...
                    >
                      <option value="all">
                        {adminLocale === "ko" ? "전체" : "All"}
                      </option>
                      <option value="suspended">
                        {adminLocale === "ko" ? "정지됨" : "Suspended"}
                      </option>
                      <option value="deleted">
                        {adminLocale === "ko" ? "탈퇴됨" : "Deleted"}
                      </option>
                    </select>
                  </div>
                  <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase">
                          ...
                          <th className="p-2.5 text-left">
                            {adminLocale === "ko" ? "이메일/ID" : "Email/ID"}
                          </th>
                          <th className="p-2.5 text-left">
                            {adminLocale === "ko" ? "전화번호" : "Phone"}
                          </th>
                      ...
                      <tbody>
                        {filteredPros.map((p) => (
                          <tr
                            key={p.pro_id}
                            className={`border-b border-gray-700/50 hover:bg-gray-700/20 cursor-pointer ${p.status === "SUSPENDED" || p.status === "DELETED" ? "opacity-40" : ""}`}
                          >
                            <td
                              className="p-2.5"
                            ...
                            <td
                              className="p-2.5 text-gray-400 text-xs font-mono"
                              onClick={() => loadUserDetail(p.pro_id, "PRO")}
                            >
                              {p.email || p.pro_id.slice(0, 12) + "..."}
                            </td>
                            ...
                            <td className="p-2.5 text-center">
                              <span
                                className={`text-xs font-bold px-2 py-0.5 rounded-full ${p.status === "DELETED" ? "bg-gray-900/50 text-gray-500 line-through" : p.status === "SUSPENDED" ? "bg-red-900/50 text-red-300" : "bg-green-900/50 text-green-300"}`}
                              >
                                {p.status === "DELETED"
                                  ? adminLocale === "ko"
                                    ? "삭제됨"
                                ...
                                {adminRole === "ADMIN" && (
                                  <>
                                    ...
                                    <button
                                      ...
                                      disabled={p.status === "DELETED"}
                                      className={`text-xs px-2 py-1 rounded font-bold ${p.status === "DELETED" ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "bg-orange-600 hover:bg-orange-700 text-white"}`}
                                    >
                                      {adminLocale === "ko" ? "환불" : "Refund"}
                                    </button>
                                  </>
                                )}
                                {/* 정지 버튼 — ADMIN, ADMIN_OPERATION 모두 표시 */}
                                {(adminRole === "ADMIN" ||
                                  adminRole === "ADMIN_OPERATION") && (
                                  <button
                                    onClick={() =>
                                      toggleSuspend(
                                        p.pro_id,
                                    ...
                                    disabled={p.status === "DELETED"}
                                    className={`text-xs px-2 py-1 rounded font-bold ${p.status === "DELETED" ? "bg-gray-700 text-gray-500 cursor-not-allowed" : p.status === "SUSPENDED" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}
                                  >
                                    {p.status === "SUSPENDED"
                                      ? adminLocale === "ko"
                                        ? "해제"
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    {adminLocale === "ko"
                      ? `${filteredPros.length}명 / 전체 ${pros.length}명`
                      : `${filteredPros.length} / ${pros.length} total`}
                  </p>
                </>
              )}
              {/* ═══ CUSTOMER MANAGEMENT ═══ */}
                ...
                <>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {filteredCustomers.map((c) => (
                          <tr
                            key={c.user_id}
                            className={`border-b border-gray-700/50 hover:bg-gray-700/20 cursor-pointer ${c.status === "SUSPENDED" || c.status === "DELETED" ? "opacity-40" : ""}`}
                          >
                            <td
                              className="p-2.5"
                              onClick={() =>
                            ...
                            <td className="p-2.5 text-gray-300 text-sm">
                              {c.phone || "-"}
                            </td>
                            ...
                            <td className="p-2.5 text-center">
                              <span
                                className={`text-xs font-bold px-2 py-0.5 rounded-full ${c.status === "DELETED" ? "bg-gray-900/50 text-gray-500 line-through" : c.status === "SUSPENDED" ? "bg-red-900/50 text-red-300" : "bg-green-900/50 text-green-300"}`}
                              >
                                {c.status === "DELETED"
                                  ? adminLocale === "ko"
                                    ? "삭제됨"
                            ...
                            <td className="p-2.5 text-center">
                              ...
                              {(adminRole === "ADMIN" ||
                                ...
                                <button
                                  ...
                                  className={`text-xs px-3 py-1 rounded font-bold ${c.status === "SUSPENDED" ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}
                                >
                                  {c.status === "SUSPENDED"
                                    ? adminLocale === "ko"
                                      ? "해제"
                  ...
                  <p className="text-xs text-gray-600 mt-2">
                    {adminLocale === "ko"
                      ...
                      : `${filteredCustomers.length} / ${customers.length} total`}
                  </p>
                </>
              )}
              {/* ═══ ADMIN MANAGEMENT ═══ */}
                ...
                <>
                  ...
                  <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 mb-6">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      ...
                      >
                        <option value="ADMIN_OPERATION">
                          {adminLocale === "ko"
                            ? "🔧 운영 관리자 (ADMIN_OPERATION)"
                            : "🔧 Operations Admin (ADMIN_OPERATION)"}
                        </option>
                        <option value="ADMIN_VIEWER">
                          {adminLocale === "ko"
                            ? "📊 뷰어 (ADMIN_VIEWER)"
                            : "📊 Viewer (ADMIN_VIEWER)"}
                        </option>
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      {adminLocale === "ko"
                        ? "※ ADMIN(최고 관리자) 등급은 이 화면에서 부여할 수 없습니다."
                        : "※ ADMIN (Super Admin) role cannot be granted from this screen."}
                    </p>
                  </div>
                      ...
                      <tbody>
                        {admins.map((a) => (
                          ...
                          >
                            ...
                            <td className="p-2.5 text-center">
                              {a.role === "ADMIN" && (
                                <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-yellow-900/50 text-yellow-300">
                                  {adminLocale === "ko"
                                    ? "👑 최고 관리자"
                                    : "👑 Super Admin"}
                                </span>
                              ...
                              {a.role === "ADMIN_VIEWER" && (
                                <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-gray-700/50 text-gray-300">
                                  {adminLocale === "ko"
                                    ? "📊 뷰어"
                                    : "📊 Viewer"}
                                </span>
                                ...
                                <button
                                  onClick={() =>
                                    setModal({
                                      ...
                                      message:
                                        adminLocale === "ko"
                                          ? "권한을 회수하면 해당 계정이 즉시 정지됩니다.\n계속하시겠습니까?"
                                          : "Revoking admin will immediately suspend the account.\nContinue?",
                                      onConfirm: () =>
                                        handleRevokeAdmin(a.user_id, a.email),
                                    })
                                  }
                ...
                <>
                  {selectedRequest ? (
                    <div>
                      ...
                      <h1 className="text-2xl font-black mb-6 text-yellow-400">
                        {adminLocale === "ko"
                          ? "🚨 CS 관제탑 (Control Tower)"
                          : "🚨 CS Control Tower"}
                      </h1>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                        ...
                        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <h2 className="text-lg font-bold text-white">
                                {(() => {
                                  ...
                                  if (
                                    da?._history &&
                                    Array.isArray(da._history)
                                  ) {
                                    const detailStep = da._history.find(
                                      (h: any) =>
                                        h.stepText
                                          ?.toLowerCase()
                                          .includes("detailed service"),
                                    );
                                    if (detailStep?.userAnswer)
                                      return detailStep.userAnswer;
                                  }
                            ...
                            <span
                              className={`text-xs font-bold px-3 py-1.5 rounded-full ${statusColor(selectedRequest.status)}`}
                            >
                              {statusLabel(selectedRequest.status)}
                            </span>
                          </div>
                          <p className="text-xs text-gray-500">
                            {adminLocale === "ko" ? "생성" : "Created"}:{" "}
                            {fmtDate(selectedRequest.created_at, adminLocale)}
                          </p>

                          {/* 고객 요청 상세 답변 — _history Q&A 형식 우선 */}
                          {selectedRequest.dynamic_answers &&
                            typeof selectedRequest.dynamic_answers ===
                              "object" &&
                            Object.keys(selectedRequest.dynamic_answers)
                              .length > 0 && (
                              <div className="mt-4 pt-4 border-t border-gray-700">
                                <button
                                  type="button"
                                  onClick={() =>
                                    setCsDetailOpen((prev) => !prev)
                                  }
                                ...
                                >
                                  ...
                                  <span
                                    className={`transition-transform duration-200 ${csDetailOpen ? "rotate-180" : ""}`}
                                  >
                                    ▼
                                  </span>
                                </button>
                                {csDetailOpen && (
                                  <div className="space-y-2">
                                    {(() => {
                                      const da =
                                        selectedRequest.dynamic_answers as Record<
                                          string,
                                          any
                                        >;
                                      ...
                                      const labelMap: Record<string, string> = {
                                        ...
                                        merged_region: "Service Area",
                                        details:
                                          "Additional Notes / Special Requests",
                                      };
                                      const workingDa = { ...da };
                                      if (
                                        workingDa.region_reg &&
                                        workingDa.region_city
                                      ) {
                          ...
                          {chatLogs.length > 0 && (
                            ...
                            >
                              {adminLocale === "ko"
                                ...
                                : `💬 Chat Log Viewer (${chatLogs.length})`}
                            </button>
                          )}
                        </div>
                        ...
                        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 flex flex-col justify-center">
                          <h3 className="text-sm font-black text-gray-400 mb-4 border-b border-gray-700 pb-2">
                            {adminLocale === "ko"
                              ? "당사자 연락처 (마스킹 해제)"
                              : "Party Contacts (Unmasked)"}
                          </h3>
                          <div className="grid grid-cols-2 gap-4">
                            <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/50">
                              <span className="text-xs font-bold text-green-400 mb-1 block">
                                Customer
                              </span>
                              <p className="text-white font-semibold text-sm">
                                {csContactInfo?.customer?.nickname ||
                                  csContactInfo?.customer?.name ||
                                  selectedRequest.customerName}
                              </p>
                              <p className="text-xs text-gray-400 mt-1">
                                {csContactInfo?.customer?.email ||
                                  (adminLocale === "ko"
                                    ? "이메일 없음"
                                    : "No email")}
                              </p>
                              <p className="text-xs text-gray-300 font-mono mt-1">
                                {csContactInfo?.customer?.phone ||
                                  (adminLocale === "ko"
                                    ? "연락처 미등록"
                                    : "No phone")}
                              </p>
                            ...
                            <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/50">
                              ...
                              {csContactInfo?.pro ? (
                                <>
                                  <p className="text-white font-semibold text-sm">
                                    {csContactInfo.pro.nickname ||
                                      csContactInfo.pro.name}
                                  </p>
                                  <p className="text-xs text-gray-400 mt-1">
                      ...
                      <h3 className="text-sm font-bold text-gray-300 mb-3 border-t border-gray-800 pt-6">
                        📝 Quotes Sent for This Request (
                        {(selectedRequest.match_quotes || []).length})
                      </h3>
                      <div className="space-y-2 mb-6">
                        {(selectedRequest.match_quotes || [])
                          .sort(
                            (a: any, b: any) =>
                                          ...
                                          (url: string, idx: number) => (
                                            ...
                                            >
                                              <img
                                                ...
                                                className="w-20 h-20 object-cover rounded-lg border border-gray-600 hover:opacity-80 transition"
                                              />
                                            </a>
                                          ),
                                        )}
                        ...
                        {(selectedRequest.match_quotes || []).length === 0 && (
                          <p className="text-center text-gray-500 py-8 bg-gray-800/50 border border-gray-800 rounded-xl border-dashed">
                            No quotes sent.
                          </p>
                        )}
                      </div>
                      ...
                      <div className="bg-red-950/20 border border-red-900/50 rounded-xl p-6 mb-6">
                        <h3 className="text-red-500 font-black mb-1 flex items-center gap-2">
                          ⚠️ DANGER ZONE (Admin Override)
                        </h3>
                        <p className="text-xs text-red-400/80 mb-4">
                          Actions here permanently affect platform records and
                        ...
                        <div className="flex flex-wrap gap-3">
                          ...
                          <button
                            onClick={() =>
                              setSuspendModal({
                                isOpen: true,
                                userId: selectedRequest.customer_id,
                                role: "CUSTOMER",
                          ...
                          {selectedRequest.matchedProName &&
                            selectedRequest.matchedProName !== "-" &&
                            !selectedRequest.matchedProName.includes("bids") &&
                            csContactInfo?.pro && (
                              <button
                                onClick={() =>
                                  setSuspendModal({
                                    isOpen: true,
                                    userId: csContactInfo.pro.user_id,
                                    role: "PRO",
                                    currentStatus: "ACTIVE",
                                  })
                                }
                    ...
                    <>
                      ...
                      <div className="bg-gray-800 border border-gray-700 rounded-xl p-4 mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                        <div className="flex flex-col">
                          <label className="text-xs text-gray-500 mb-1 font-bold">
                            Search
                          </label>
                          <input
                            type="text"
                        ...
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {filteredRequests.map((r) => (
                              <tr
                                key={r.request_id}
                                className="border-b border-gray-700/50 hover:bg-gray-700/30 transition-colors whitespace-nowrap"
                              >
                                <td className="p-3">
                                  <div className="font-semibold text-white">
                                    {(() => {
                                ...
                                <td className="p-3 text-right font-semibold text-white">
                                  {r.status === "MATCHED" && r.matchedPrice > 0
                                    ? `${fmtNum(r.matchedPrice)}`
                                    : "-"}
                                </td>
                                ...
                                <td className="p-3 text-right">
                                  <button
                                    ...
                                    className="text-xs bg-gray-700 hover:bg-gray-600 border border-gray-600 text-white px-3 py-1.5 rounded-lg font-bold transition"
                                  >
                                    View Detail
                                  </button>
                                </td>
                ...
                <>
                  ...
                  <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase">
                          <th className="p-2.5 text-left">Customer</th>
                          <th className="p-2.5 text-left">Pro</th>
                          <th className="p-2.5 text-center">Rating</th>
                          <th className="p-2.5 text-left">Comment</th>
                      ...
                      <tbody>
                        {filteredReviews.map((r) => (
                          <tr
                            key={r.review_id}
                            className={`border-b border-gray-700/50 hover:bg-gray-700/20 ${r.is_hidden ? "opacity-40" : ""}`}
                          >
                            <td className="p-2.5 text-white text-sm">
                              {r.customerName}
                            </td>
                            <td className="p-2.5 text-gray-300 text-sm">
                              {r.proName}
                              ...
                              <div className="flex items-center justify-center gap-1">
                                <div className="flex items-center gap-0.5">
                                  {[1, 2, 3, 4, 5].map((s) => (
                                    <span
                                      key={s}
                                      className={`text-xs ${s <= r.rating ? "text-yellow-400" : "text-gray-600"}`}
                                    >
                                      ★
                                    </span>
                                  ))}
                            ...
                            <td className="p-2.5 text-gray-400 text-xs max-w-xs truncate">
                              {r.comment || "(none)"}
                            </td>
                            <td className="p-2.5 text-center text-gray-500 text-xs">
                              {fmtDate(r.created_at)}
                            </td>
                            <td className="p-2.5 text-center">
                              <div className="flex items-center justify-center gap-1">
                                    ...
                                    <button
                                      onClick={() =>
                                        isEligible
                                          ...
                                          : alert(
                                              "Only reviews with rating 4.5+ and 50+ characters can be featured on the main page.",
                                            )
                                      }
                                      ...
                                      title={
                                        !isEligible
                                          ? "Not eligible (requires 4.5+ rating & 50+ characters)"
                                          : "Toggle main page featured"
                                      }
                                    >
                                      {r.is_featured_on_main
                                        ? "★ Featured"
                                ...
                                <button
                                  ...
                                  className={`text-xs px-2 py-1 rounded font-bold ${r.is_hidden ? "bg-green-600 text-white" : "bg-yellow-600 text-white"}`}
                                >
              ...
              {tab === "search_logs" && (
                <>
                  <h1 className="text-2xl font-black mb-4">
                    🔍 Search / Traffic Analysis (Zero-Search)
                  </h1>
                  <p className="text-sm text-gray-400 mb-6">
                    These are keywords that returned 0 results. Valid keywords
                    can be mapped to a specific category so future searches
                    return results.
                  </p>
                        ...
                        {searchLogsLoading ? (
                          ...
                          <tr>
                            <td
                              ...
                              className="p-12 text-center text-gray-500"
                            >
                              No zero-result search logs found. (Zero Search
                              clean zone)
                            </td>
                          </tr>
                        ) : (
                            ...
                            >
                              ...
                              <td className="p-3 text-right flex items-center justify-end gap-2">
                                <select
                                  ...
                                  onChange={(e) =>
                                    setSelectedCategoryMapping((prev) => ({
                                      ...prev,
                                ...
                                <button
                                  onClick={() =>
                                    handleIgnoreSearchKeyword(log.keyword)
                                  }
              ...
              {tab === "settings" && (
                <>
                  <h1 className="text-2xl font-black mb-6">
                    ⚙️ Billing Controller
                  </h1>
                  <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 space-y-6">
                    {[
                      {
                        key: "quote_cost",
                        t: "Quote Sending Cost",
                        d: "Credits deducted from a pro per quote sent",
                      },
                      {
                        key: "max_quotes_per_request",
                        t: "Max Quotes per Request",
                        d: "Maximum number of quotes receivable per request",
                      },
                      {
                        key: "signup_bonus",
                        t: "New Member Signup Bonus",
                        d: "Credits automatically granted to pros on signup",
                      },
                    ].map((s, i) => (
                      <div
                        key={s.key}
                        className={i > 0 ? "border-t border-gray-700 pt-6" : ""}
                      >
                        <h3 className="text-base font-bold mb-1">{s.t}</h3>
                        <p className="text-gray-400 text-sm mb-3">
                          {s.d} (Current:{" "}
                          <span className="text-white font-bold">
                            {fmtNum(platformSettings[s.key] ?? 0)}
                          </span>
                          )
                        </p>
                        <div className="flex items-center gap-3">
                          <input
                            type="number"
                            value={
                              settingsInputs[s.key] ??
                              String(platformSettings[s.key] ?? 0)
                            }
                            onChange={(e) =>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              if (error) {
                                ...
                                alert(
                                  `${s.t} has been saved as ${fmtNum(val)}.`,
                                );
                              }
                              setSavingKey(null);
                            }}
                  ...
                  <div className="mt-8 bg-gradient-to-br from-red-900/30 to-red-800/20 rounded-xl border-2 border-red-600/50 p-6">
                    <h2 className="text-lg font-black text-red-400 mb-2 flex items-center gap-2">
                      <span className="text-2xl">⚠️</span> DANGER ZONE — Reset
                      Test Data
                    </h2>
                    <p className="text-sm text-gray-400 mb-4 leading-relaxed break-keep">
                      ...
                      all activity records (requests, quotes, chats, reviews,
                      payments, notifications, sanction history, reports,
                      inquiries, templates) and resets cash and statistics to 0.
                      <br />
                      Member accounts (users, auth.users, pro_profiles) are 100%
                      preserved. An automatic backup snapshot is created before
                      deletion.
                    </p>
                    <button
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="relative bg-gray-900 rounded-2xl border-2 border-red-600/60 shadow-2xl max-w-md w-full mx-4 p-6">
                        <div className="text-center mb-5">
                          ...
                          <p className="text-sm text-gray-400 mt-2 break-keep">
                            ...
                            <strong className="text-white">
                              all activity data from 10 tables
                            </strong>
                            <br />
                        ...
                        <div className="flex gap-3">
                          ...
                          <button
                            disabled={
                              resetConfirmText !== "CONFIRM RESET" || resetting
                            }
                            onClick={async () => {
                              setResetting(true);
                              try {
                                ...
                                showToast(
                                  "All test data has been reset (including cash/transactions/reports). Backup snapshots preserved in backup_*_temp tables.",
                                  "success",
                                );
                              } catch (e: any) {
                                showToast(
              ...
              {tab === "cms" && (
                <>
                  ...
                  <div className="space-y-8">
                    ...
                    <p className="text-sm text-gray-400 mb-6">
                      Replace the large banner (image or video) on the right
                      side of the customer funnel.
                    </p>
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                      <div className="w-full md:w-1/2">
                        {cmsBanners[0] ? (
                          ...
                          <div className="rounded-xl border border-dashed border-gray-600 aspect-video flex items-center justify-center text-gray-500">
                            No banner
                          </div>
                        )}
                      </div>
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const optimizedFile =
                                mType === "IMAGE"
                                  ? await optimizeImage(file, 1920, 1080, 0.8)
                                  : file;
                              const ext =
                                optimizedFile.name.split(".").pop() || "png";
                              const fileName = `home_banner_${Date.now()}.${mType === "IMAGE" ? "webp" : ext}`;

                              const { error: upErr } = await supabase.storage
                                .from("quote_images")
                                .upload(`cms/${fileName}`, optimizedFile);
                              if (upErr) throw upErr;
                              const {
                                data: { publicUrl },
                              } = supabase.storage
                                .from("quote_images")
                                .getPublicUrl(`cms/${fileName}`);
                    ...
                    <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                      <div className="flex justify-between items-center mb-6">
                        <div>
                          ...
                          <p className="text-sm text-gray-400 mt-1">
                            Manage the category buttons at the bottom of the
                            homepage.
                          </p>
                        </div>
                      </div>
                      <div className="overflow-x-auto">
                          ...
                          <tbody>
                            {cmsCategories.map((cat, idx) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  <span
                                    className={`px-2 py-1 rounded text-xs font-bold ${cat.is_active ? "bg-green-900/50 text-green-400" : "bg-red-900/50 text-red-400"}`}
                                  >
                            ...
                            {cmsCategories.length === 0 && (
                              <tr>
                                ...
                                >
                                  No category data. Please complete initial
                                  setup via SQL.
                                </td>
                              </tr>
                            )}
                        ...
                        <p className="text-xs text-gray-500 mt-4">
                          * Category configuration changes (order, text) are
                          currently managed directly in the cms_categories table
                          via a DB client such as DataGrip. (Editor UI coming
                          soon)
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                      <div className="flex justify-between items-center mb-6">
                        <div>
                          <h2 className="text-xl font-bold">
                            Support Page Management (CMS V2)
                          </h2>
                          <p className="text-sm text-gray-400 mt-1">
                            Directly manage 2-depth text documents such as
                            platform introductions and customer guides.
                          </p>
                        </div>
                      </div>
                        ...
                        <div className="overflow-x-auto">
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3 text-center">
                                      <span
                                        className={`px-2 py-1 rounded text-xs font-bold ${page.is_active ? "bg-green-900/50 text-green-400" : "bg-red-900/50 text-red-400"}`}
                                      >
                                        {page.is_active
                                          ? "Published"
                                          : "Hidden"}
                          ...
                          {supportPages.length === 0 && (
                            <div className="text-center p-8 text-gray-500">
                              No 2-depth pages registered.
                            </div>
                          )}
                        </div>
                      ) : (
                        <form
                          onSubmit={handleSaveSupportPage}
                              ...
                              <span className="text-gray-500 text-sm ml-2">
                                (/support/
                                {
                                  supportCategories.find(
                                    (c) =>
                                      c.id === editingSupportPage.category_id,
                                  )?.slug
                                }
                                /{editingSupportPage.slug})
                              </span>
                            </h3>
                            <button
                              type="button"
                              onClick={() => setEditingSupportPage(null)}
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="pt-4 flex justify-end gap-2">
                              <button
                                type="button"
                                onClick={() => setEditingSupportPage(null)}
                                className="px-4 py-2 border border-gray-600 text-gray-400 font-bold rounded hover:bg-gray-800 transition"
                              >
                                Cancel
                              </button>
                              <button
                    ...
                    <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
                      <div className="flex justify-between items-center mb-6">
                        <div>
                          <h2 className="text-xl font-bold">
                            Legal Document Management (Version History)
                          </h2>
                          <p className="text-sm text-gray-400 mt-1">
                            Manage documents requiring version history such as
                      ...
                      {!editingLegalDoc ? (
                        <div className="overflow-x-auto">
                            ...
                            <tbody>
                              {legalDocs.map((doc) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 text-center">
                                    <span
                                      className={`px-2 py-1 rounded text-xs font-bold ${doc.is_active ? "bg-purple-900/50 text-purple-400" : "bg-gray-700 text-gray-400"}`}
                                    >
                                      {doc.is_active
                                        ? "Active"
                                        : "Previous/Pending"}
                          ...
                          {legalDocs.length === 0 && (
                            <div className="text-center p-8 text-gray-500">
                              No legal documents registered.
                            </div>
                          )}
                        </div>
                      ) : (
                        <form
                          onSubmit={handleSaveLegalDoc}
                          className="bg-gray-900/50 p-6 rounded-xl border border-gray-700"
                        >
                          <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-lg text-white">
                              Legal Document{" "}
                              {editingLegalDoc.id ? "Edit" : "Create"}{" "}
                              <span className="text-gray-500 text-sm ml-2">
                                (/legal/
                                {editingLegalDoc.document_type.toLowerCase()})
                              </span>
                            </h3>
                            <button
                              type="button"
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-2 bg-purple-900/30 p-3 rounded border border-purple-900/50">
                              ...
                              />
                              <label
                                htmlFor="legal-active"
                                className="text-sm font-bold text-purple-300"
                              >
                                Publish this document as the current active
                                version. (Will replace existing versions)
                              </label>
                            </div>
                ...
                <>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase">
                            ...
                            <th className="p-3 text-left flex-1 min-w-[200px]">
                              Title
                            </th>
                          ...
                          {inquiries.map((iq) => (
                            ...
                            >
                              ...
                              <td className="p-3 text-white font-medium line-clamp-1 break-all flex-1">
                                {iq.title}
                                {Array.isArray(iq.image_urls) &&
                                  iq.image_urls.length > 0 && (
                                    <span className="ml-2 text-xs text-gray-500">
                                      📎 {iq.image_urls.length}
                                    </span>
                                  )}
                              </td>
                              ...
                              <td className="p-3 text-right">
                                {iq.status === "resolved" ? (
                                  <button
                                    ...
                                    className="text-green-300 bg-green-900/30 border border-green-900/60 hover:bg-green-700 hover:text-white px-3 py-1.5 rounded transition text-xs font-bold"
                                  >
                                    View Reply
                                  </button>
                                ) : (
                          ...
                          {inquiries.length === 0 && (
                            <tr>
                              <td
                                ...
                                className="p-12 text-center text-gray-500"
                              >
                                No inquiries found. Please check the selected
                                filters.
                              </td>
                            </tr>
                          )}
                  ...
                  {inquiriesTotalCount > INQUIRIES_PAGE_SIZE && (
                    <div className="flex items-center justify-between px-4 py-3 border-t border-gray-700">
                      <span className="text-xs text-gray-500">
                        Total {inquiriesTotalCount} · Page {inquiriesPage} /{" "}
                        {Math.ceil(inquiriesTotalCount / INQUIRIES_PAGE_SIZE)}
                      </span>
                      <div className="flex gap-1">
                        <button
                          onClick={() => loadInquiries(inquiriesPage - 1)}
                          disabled={inquiriesPage <= 1 || inquiriesLoading}
                          className="px-3 py-1.5 text-xs bg-gray-700 hover:bg-gray-600 disabled:opacity-30 text-white rounded transition"
                        >
                          ← Prev
                        </button>
                        {Array.from(
                          ...
                          (_, i) => {
                            ...
                            const startPage = Math.max(
                              1,
                              Math.min(inquiriesPage - 2, totalPages - 4),
                            );
                            const p = startPage + i;
                              ...
                              >
                                ...
                                <td className="p-2.5 text-center">
                                  ...
                                  {p.status === "HELD" && p.hold_reason && (
                                    <p className="text-xs text-yellow-400 mt-1 max-w-[160px] break-words">
                                      {p.hold_reason}
                                    </p>
                                  )}
                                  {p.admin_note && (
                                    ...
                                    <div className="flex flex-col gap-1 items-center">
                                      ...
                                      <div className="flex gap-1">
                                        <button
                                          ...
                                          onClick={async () => {
                                            if (!payoutNote[p.id]?.trim()) {
                                              alert(
                                                'Please enter the account holder identity verification memo before approving.\ne.g., "Identity verified - John Doe"',
                                              );
                                              return;
                                            }
                                            if (
                                              !window.confirm(
                                                `Have you verified that the account holder [${p.account_holder}] matches the registered name?\n\nClick OK to approve.`,
                                              )
                                            )
                                              return;
              ...
              {tab === "audit_log" && (
                <>
                  ...
                  <p className="text-gray-500 text-xs mb-4">
                    A record of all major actions performed by admins. Tracks
                  ...
                  <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                    ...
                    <button
                      onClick={() => loadAuditLogs(auditPage, auditFilter)}
                      className="text-xs text-gray-400 hover:text-white flex items-center gap-1"
                    >
                      🔄 Refresh
                    </button>
                  ...
                  <p className="text-xs text-gray-500 mb-3">
                    Total {auditTotal.toLocaleString()}
                  </p>
                        ...
                        {auditLoading ? (
                          ...
                          auditLogs.map((log, i) => (
                            ...
                            >
                              ...
                              <td className="p-2.5 text-gray-300 text-xs max-w-xs truncate">
                                {log.reason || "-"}
                              </td>
                            </tr>
                          ))
                    ...
                    <div className="flex justify-center gap-2 mt-4">
                      <button
                        ...
                        onClick={() => {
                          const p = auditPage - 1;
                          setAuditPage(p);
                          loadAuditLogs(p, auditFilter);
                        }}
                        className="px-3 py-1.5 text-xs bg-gray-800 border border-gray-700 rounded-lg text-gray-400 hover:text-white disabled:opacity-40"
                      >
                        ← Prev
                      </button>
      ...
      {selectedInquiry && (
        ...
        >
          <div
            className="bg-gray-800 rounded-2xl w-full max-w-2xl border border-gray-700 shadow-2xl flex flex-col max-h-[90vh]"
            onClick={(e) => e.stopPropagation()}
          >
              ...
              <div>
                ...
                <div className="flex gap-4 text-xs text-gray-400 mb-4 bg-gray-900/50 p-3 rounded-lg border border-gray-700/50">
                  <p>
                    ...
                    {selectedInquiry.users?.nickname ||
                      selectedInquiry.users?.name ||
                      "Unknown"}{" "}
                    ({selectedInquiry.user_type})
                  </p>
                  <p>
                    <strong>Type:</strong> {selectedInquiry.category}
                  </p>
                  <p>
                    <strong>Date:</strong> {fmtDate(selectedInquiry.created_at)}
                  </p>
                </div>
                <p className="text-gray-300 text-sm whitespace-pre-wrap bg-gray-700/30 p-4 rounded-xl leading-relaxed border border-gray-700/30">
                  {selectedInquiry.content}
                </p>
                {Array.isArray(selectedInquiry.image_urls) &&
                  selectedInquiry.image_urls.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {selectedInquiry.image_urls.map(
                        (url: string, idx: number) => (
                          <a
                            key={idx}
                            href={url}
              ...
              <div className="border-t border-gray-700 pt-6">
                ...
                <textarea
                  ...
                  placeholder={
                    selectedInquiry.status === "resolved"
                      ? "No reply registered."
                      : "Write the reply to be sent to the customer. (HTML tags supported)"
                  }
                ...
                {/* 기존 등록된 답변 이미지 표시 */}
                {Array.isArray(selectedInquiry.admin_reply_images) &&
                  selectedInquiry.admin_reply_images.length > 0 && (
                    <div className="mt-3">
                      <p className="text-xs font-bold text-gray-400 mb-2">
                        📎 Registered Reply Images
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {selectedInquiry.admin_reply_images.map(
                          (url: string, idx: number) => (
                            <a
                              key={idx}
                              href={url}
                  ...
                  {replyImagePreviews.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-2">
                      {replyImagePreviews.map((src, idx) => (
                        <div
                          key={idx}
                          className="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-600"
                        >
                          <img
                            src={src}
                            alt={`Reply attachment ${idx + 1}`}
            ...
            <div className="p-5 border-t border-gray-700 flex justify-between items-center bg-gray-800/80 rounded-b-2xl">
              ...
              <div className="flex gap-2">
                {selectedInquiry.status !== "pending" && (
                  <button
                    onClick={() =>
                      handleUpdateInquiryStatus(selectedInquiry.id, "pending")
                    }
                ...
                {selectedInquiry.status !== "resolved" && (
                  <button
                    onClick={() =>
                      handleUpdateInquiryStatus(selectedInquiry.id, "resolved")
                    }
                    disabled={inquiryStatusUpdating}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-5 py-2.5 border border-gray-600 text-gray-300 hover:bg-gray-700 font-bold rounded-lg transition text-sm"
                >
                  Close
                </button>
                {selectedInquiry.status !== "resolved" && (
          ...
          <div className="bg-gray-800 rounded-2xl w-full max-w-md border border-gray-700 shadow-2xl">
            <div className="p-5 border-b border-gray-700">
              <h3 className="text-lg font-bold">
                {cashModal.type === "charge"
                  ? "💎 Credits Top-up"
                  : "🔥 Credits Refund/Deduct"}
              </h3>
              <p className="text-sm text-gray-400 mt-1">
                Target:{" "}
                <span className="text-white font-semibold">
                  {cashModal.pro.nickname ||
                    cashModal.pro.name ||
                    cashModal.pro.pro_id.slice(0, 12)}
                </span>
              </p>
              <p className="text-sm text-gray-400">
                Available Credits:{" "}
            ...
            <div className="p-5 space-y-3">
              ...
              <div>
                <label className="text-xs text-gray-400 font-bold mb-1 block">
                  {cashModal.type === "charge" ? "Top-up" : "Refund/Deduct"}{" "}
                  Amount
                </label>
                <input
                  type="number"
              ...
              {cashAmount && Number(cashAmount) > 0 && (
                <div className="bg-gray-900 rounded-lg p-3 text-sm space-y-1">
                  <div className="flex justify-between text-gray-400">
                    <span>
                      {cashType === "BONUS" ? "Bonus Credits" : "Real Credits"}
                    </span>
                    <span>
                      {fmtNum(
                        cashType === "BONUS"
                          ? cashModal.pro.bonus_cash || 0
                          : cashModal.pro.current_cash,
                      )}
                    </span>
                  </div>
                  <div
            ...
            <div className="p-5 border-t border-gray-700 flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-white font-bold text-sm"
              >
                Cancel
              </button>
              <button
      ...
      {drilldown && (
        ...
        >
          <div
            className="bg-gray-800 rounded-2xl w-full max-w-3xl border border-gray-700 shadow-2xl max-h-[85vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 border-b border-gray-700 flex justify-between items-start">
              <div>
                <h3 className="text-lg font-bold">
                  🔍 {drilldown.proName} Credit History
                </h3>
                <p className="text-xs text-gray-400 mt-1">
                  {drilldown.proEmail}
                  {drilldown.proPhone ? ` · ${drilldown.proPhone}` : ""}
                </p>
              </div>
              <button
                onClick={() => setDrilldown(null)}
              ...
              <>
                ...
                {(() => {
                  ...
                  const sumByType = (types: string[]) =>
                    txs
                      .filter((t) => types.includes(t.tx_type))
                      .reduce((s, t) => s + Number(t.amount), 0);
                  const stats = [
                    {
                      l: "Total",
                      v: txs.reduce((s, t) => s + Number(t.amount), 0),
                      c: "text-white",
                    },
                    {
                      l: "Credits Used",
                      v: sumByType(["DEDUCT_QUOTE"]),
                      c: "text-red-400",
                    },
                    {
                      l: "Top-up",
                    ...
                    {
                      l: "Refund",
                      v: sumByType(["REFUND", "ADMIN_REFUND"]),
                      c: "text-orange-400",
                    },
                  ];
                  return (
                ...
                <div className="overflow-auto flex-1">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-700 text-gray-400 text-xs uppercase sticky top-0 bg-gray-800">
                        <th className="p-2.5 text-left">Time</th>
                    ...
                    <tbody>
                      {drilldown.txs
                        .filter((tx) => {
                          ...
                          if (drilldownFilter === "CHARGE")
                            return ["CHARGE", "ADMIN_CHARGE", "BONUS"].includes(
                              tx.tx_type,
                            );
                          if (drilldownFilter === "REFUND")
                            return ["REFUND", "ADMIN_REFUND"].includes(
                              tx.tx_type,
                            );
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[80vh]">
            ...
            <div className="p-4 overflow-y-auto flex-1 flex flex-col-reverse gap-3">
              {chatLogs.map((msg: any) => {
                const isCustomer =
                  msg.sender_id === selectedRequest.customer_id;
                return (
                  <div
                    key={msg.message_id}
      ...
      {suspendModal?.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-gray-900 border border-red-900/50 rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden shadow-red-900/20">
            <div className="bg-red-950/40 p-5 border-b border-red-900/30">
              <h2 className="text-red-400 font-bold text-lg mb-1">
                Enter Account Suspension Reason (Audit Log)
              </h2>
              <p className="text-xs text-red-300/70 leading-relaxed">
                The reason entered will be permanently stored in the database
                audit log (suspension_reason).
              </p>
            </div>
            <div className="p-5">
              <div className="mb-4">
                <label className="block text-xs font-bold text-gray-400 mb-2">
                  CS Sanction Reason (min. 5 characters)
                </label>
                <textarea
                  value={suspendReason}
              ...
              <div className="flex gap-2 justify-end mt-6">
                ...
                <button
                  onClick={handleSuspendUserWithReason}
                  className={`px-5 py-2.5 rounded-lg text-sm font-bold transition ${suspendReason.length >= 5 ? "bg-red-600 hover:bg-red-500 text-white" : "bg-red-900/50 text-red-500/50 cursor-not-allowed"}`}
                >
                  Execute Suspension
                </button>
              </div>
      ...
      {csChatModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div
            className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col"
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 p-4 space-y-2">
              {csChatRoomId === undefined && csChatLoading ? (
                ...
                <p className="text-center text-gray-600 py-10 text-xs bg-gray-800/50 border border-gray-800 rounded-xl border-dashed">
                  No chat room yet.
                </p>
              ) : (
                <>
                  ...
                  {csChatMessages.length === 0 && !csChatLoading ? (
                    <p className="text-center text-gray-600 py-6 text-xs">
                      No chat history.
                    </p>
                  ) : (
                    [...csChatMessages].reverse().map((msg: any) => {
                      const senderName =
                        ...
                        msg.sender_id?.slice(0, 8) + "...";
                      const isCustomer =
                        msg.sender_id === csChatModal.customerId;
                      return (
                        <div
                          key={msg.message_id}
                    ...
                    <div className="flex items-center gap-4 bg-gray-800 p-4 rounded-xl border border-gray-700">
                      ...
                      <div>
                        <p className="text-white font-bold text-lg">
                          {proDetailData.nickname || proDetailData.name || "-"}
                        </p>
                        <p className="text-gray-400 text-xs mt-0.5">
                          PRO · {proDetailData.status || "ACTIVE"}
                        </p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-yellow-400 text-xs">
                            ⭐{" "}
                            {Number(proDetailData.average_rating || 0).toFixed(
                              1,
                            )}
                          </span>
                          <span className="text-gray-500 text-xs">
                            ({proDetailData.review_count || 0} reviews)
                          </span>
                        </div>
                      </div>
                      <div className="ml-auto text-right">
                    ...
                    <div className="bg-gray-800 p-4 rounded-xl border border-gray-700">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <p
                            className={`text-sm font-bold ${proDetailData.is_phone_verified ? "text-green-400" : "text-gray-500"}`}
                          >
                            {proDetailData.is_phone_verified
                              ? "✅ Verified"
                              : "Unverified"}
                          </p>
                        </div>
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    {proDetailTabData.ledger == null ? (
                      ...
                      <div className="space-y-1.5">
                        {(proDetailTabData.ledger || []).map(
                          (row: any, i: number) => (
                            <div
                              key={i}
                              className="flex items-center justify-between bg-gray-800/60 px-3 py-2.5 rounded-lg border border-gray-700/50"
                            >
                              ...
                              <div className="text-right ml-3 flex-shrink-0">
                                <p
                                  className={`text-sm font-bold ${row.amount >= 0 ? "text-green-400" : "text-red-400"}`}
                                >
                                  {row.amount >= 0 ? "+" : ""}
                        ...
                        {proDetailLedgerHasMore && (
                          <button
                            onClick={() =>
                              handleProDetailLedgerMore(proDetailModal!.proId)
                            }
                ...
                {proDetailTab === "quotes" && (
                  <div className="p-5 space-y-3">
                    {proDetailTabLoading ? (
                      ...
                      <div className="space-y-1.5">
                        {(proDetailTabData.quotes || []).map(
                          (q: any, i: number) => (
                            <div
                              key={i}
                              className="bg-gray-800/60 px-3 py-2.5 rounded-lg border border-gray-700/50"
                            >
                  ...
                  <div className="p-5 space-y-3">
                    <div className="flex items-center gap-2 bg-gray-800 p-3 rounded-xl border border-gray-700">
                      <span className="text-yellow-400 font-bold">
                        ⭐{" "}
                        {Number(proDetailData.average_rating || 0).toFixed(1)}
                      </span>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-[#1e2433] rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 flex flex-col items-center gap-4">
            ...
            <p className="text-gray-400 text-sm text-center leading-relaxed whitespace-pre-line">
              {modal.message}
            </p>
            {modal.type === "confirm" ? (
      ...
      {suspendReasonModal?.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <div className="bg-[#1e2433] rounded-2xl shadow-2xl p-8 w-full max-w-md mx-4 flex flex-col gap-4">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${suspendReasonModal.currentStatus === "SUSPENDED" ? "bg-green-500/20" : "bg-red-500/20"}`}
            >
              ...
              <button
                onClick={() => {
                  if (
                    ...
                    suspendReasonModal.reason.trim().length < 5
                  ) {
                    setModal({
                      type: "error",
                      title: "Input Error",
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-[#1e2433] rounded-2xl shadow-2xl p-8 w-full max-w-sm mx-4 flex flex-col gap-4 text-center">
            ...
            <p className="text-gray-400 text-sm">
              You will be automatically logged out in 30 seconds due to
              inactivity.
            </p>
            <button
...
export default function AdminDashboardPage() {
  return (
    <React.Suspense
      fallback={
        <div className="min-h-screen bg-[#0E121E] flex items-center justify-center text-white">
          Loading...
        </div>
      }
    >
