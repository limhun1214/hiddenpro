...
export default function HomePage() {
    ...
    const [allServices, setAllServices] = useState<{ service: string, category: string, depth2: string }[]>([]);
    ...
    React.useEffect(() => {
        const loadCategories = async () => {
            ...
            if (data) {
                const list: { service: string, category: string, depth2: string }[] = [];
                const d1Set = new Set<string>();
                data.forEach(item => {
                    if (item.depth1) {
                        d1Set.add(item.depth1);
                        list.push({ service: item.name, category: item.depth1, depth2: item.depth2 || '' });
                    }
                });
                setAllServices(list);
    ...
    const [cmsCategories, setCmsCategories] = useState<{ id: string; title: string; desc: string; icon: string; link_url: string }[]>([]);
        ...
        const fetchCMS = async () => {
            try {
                ...
                if (cats && cats.length > 0) {
                    setCmsCategories(cats.map(c => ({ id: c.id, title: c.title, desc: c.description || '', icon: c.icon || '✨', link_url: c.link_url })));
                }
    ...
    const fallbackCategories = depth1Keys.map((key, idx) => ({
        ...
        desc: categoryUI[key]?.desc || ' HiddenPro 서비스',
                ...
                <div className="fixed top-0 left-0 w-full z-[200] bg-red-600 text-white py-4 px-6 shadow-lg animate-slide-down">
                    <div className="max-w-4xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="font-bold text-lg">관리자에 의해 이용이 정지된 계정입니다.</p>
                                <p className="text-red-100 text-sm">문의: support@hiddenpro.kr</p>
                            </div>
                        </div>
                ...
                <div className="fixed top-0 left-0 w-full z-[200] bg-gray-800 text-white py-4 px-6 shadow-lg animate-slide-down">
                    <div className="max-w-4xl mx-auto flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <span className="text-2xl">👋</span>
                            <div>
                                <p className="font-bold text-lg">탈퇴가 완료되었습니다.</p>
                                <p className="text-gray-300 text-sm">그동안 HiddenPro를 이용해 주셔서 감사합니다.</p>
                            </div>
                        </div>
                ...
                <div className="max-w-6xl mx-auto w-full relative z-30 flex flex-col md:flex-row items-center justify-between min-h-[380px] lg:min-h-[440px]">
                    ...
                    <div className="text-left flex flex-col justify-center w-full md:w-3/5 lg:w-1/2">
                        <h1 className="text-[28px] md:text-[36px] lg:text-[42px] text-white tracking-[-1.5px] leading-[1.3] break-keep mb-6 drop-shadow-md">
                            <span className="font-normal">필리핀 최고의 홈서비스,</span><br />
                            <span className="font-bold">완벽한 전문가를 만나보세요.</span>
                        </h1>
                        <p className="text-lg sm:text-[22px] text-gray-200 font-medium mb-12 max-w-lg leading-[1.6] tracking-[-0.5px] drop-shadow-sm">
                            전문가의 생생한 리뷰를 확인하고,<br className="hidden sm:block" /> 안전하게 거래하세요.
                        </p>
                        ...
                        <div className="relative max-w-xl group z-50" ref={searchRef}>
                            <input
                                ...
                                placeholder={animatedPlaceholder || '어떤 서비스가 필요하신가요?'}
                            ...
                            {showDropdown && (
                                <div className="absolute top-full left-0 w-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-[300px] overflow-y-auto z-[60]">
                                    {isSearching ? (
                                        <div className="px-6 py-6 text-center text-gray-500 font-medium">검색 중...</div>
                                    ) : searchQuery.trim() ? (
                                        <>
                                        ...
                                        <>
                                            <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                                                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">🔥 추천 서비스</span>
                                                {showAllRecommendServices && (
                                                    <span onClick={(e) => { e.stopPropagation(); setShowAllRecommendServices(false); }} className="text-xs font-bold text-gray-400 hover:text-gray-600 cursor-pointer">접기 ∧</span>
                                                )}
                                            </div>
                                            {(showAllRecommendServices ? allServices : [
                                                { service: '에어컨 청소', category: '이사/청소' },
                                                { service: '가사/메이드', category: '이사/청소' },
                                                { service: '수도/배관 및 워터펌프 수리', category: '설치/수리' },
                                                { service: '전기 및 발전기 수리', category: '설치/수리' },
                                                { service: '특수 청소 및 방역', category: '이사/청소' },
                                            ]).map((item, i) => (
                                                <div
                                                    key={i}
                                                    onClick={() => { setShowDropdown(false); router.push(`/request?categoryId=${encodeURIComponent(item.category)}&serviceId=${encodeURIComponent(item.service)}${(item as any).depth2 ? `&depth2=${encodeURIComponent((item as any).depth2)}&serviceType=${encodeURIComponent(item.service)}` : ''}`); }}
                    ...
                    <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
                        ...
                        <div className="hidden md:block absolute top-[5%] md:top-[10%] right-[2%] sm:right-[5%] md:right-[15%] lg:right-[20%] pointer-events-auto animate-pulse-soft">
                            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/20 rounded-full px-4 sm:px-5 py-2 sm:py-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.15)] scale-75 sm:scale-90 md:scale-100 origin-top-right">
                                ...
                                <span className="text-white font-bold text-xs sm:text-sm tracking-tight drop-shadow-md">100% 신원 검증 완료</span>
                            </div>
                        </div>
                        ...
                        <div className="hidden md:block absolute top-[35%] right-[-5%] sm:right-[2%] md:right-[-2%] lg:right-[5%] pointer-events-auto animate-float-1">
                            <div className="flex flex-col gap-2 bg-white/15 backdrop-blur-lg border border-white/20 rounded-2xl p-3 sm:p-4 shadow-[0_12px_40px_rgba(0,0,0,0.2)] scale-75 sm:scale-90 md:scale-100 origin-right">
                                <div className="flex items-center gap-3">
                                    <img src="https://api.dicebear.com/7.x/notionists/svg?seed=Pro&backgroundColor=ffedd5" alt="고수" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-white/30" />
                                    <div>
                                        ...
                                        <div className="text-white font-bold text-[11px] sm:text-[13px] tracking-tight drop-shadow-md">친절하고 완벽해요!</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="hidden md:block absolute bottom-[25%] md:bottom-[15%] right-[10%] sm:right-[15%] md:right-[5%] lg:right-[15%] pointer-events-auto animate-fade-up-delay">
                            <div className="flex items-center gap-3 bg-white/15 backdrop-blur-xl border border-white/20 rounded-[20px] px-4 sm:px-6 py-3 sm:py-4 shadow-[0_16px_40px_rgba(0,0,0,0.25)] scale-75 sm:scale-90 md:scale-100 origin-bottom-right animate-float-2">
                                ...
                                <span className="text-white font-extrabold text-[13px] sm:text-[15px] tracking-tight drop-shadow-md">3분 만에 예약 완료</span>
                            </div>
                        </div>
                ...
                <div className="max-w-6xl mx-auto w-full relative z-10 flex flex-col items-center mt-12 mb-4">
                    ...
                    <div className="w-full grid grid-cols-3 sm:grid-cols-6 gap-3 md:gap-6 lg:gap-8 px-0">
                        {displayCategories.map((cat) => (
                            <Link key={cat.id} href={cat.link_url || `/quotes/requests/request?categoryId=${encodeURIComponent(cat.title)}`} className="group flex flex-col items-center justify-start h-full">
            ...
            <section className="py-20 bg-slate-50 border-t border-b border-gray-100">
                <div className="max-w-6xl mx-auto px-4 sm:px-6">
                    <div className="flex justify-between items-end mb-10">
                        <div>
                            <h2 className="text-[28px] md:text-[36px] font-black text-[#1A1C23] tracking-[-1px] mb-2">실시간 생생한 리뷰</h2>
                            <p className="text-[#555969] font-medium text-lg">방금 전까지 진행된 실제 고객들의 생생한 리뷰입니다.</p>
                        </div>
                    </div>

                    {isReviewsLoading ? (
                        <div className="flex justify-center py-10">
                            <span className="text-gray-400 loading animate-pulse">실시간 데이터를 불러오는 중...</span>
                        </div>
                                    ...
                                    <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="flex flex-col">
                                                <span className="text-[15px] font-extrabold text-[#111827]">{review.users?.nickname || review.users?.name || '익명 고객'}</span>
                                                <span className="text-xs text-blue-600 font-bold">{review.chat_rooms?.match_requests?.service_type || '홈서비스'}</span>
                                            </div>
                                        </div>
                                        <span className="text-[12px] text-gray-400 font-medium bg-slate-50 px-2.5 py-1 rounded-md">{new Date(review.created_at).toLocaleDateString('ko-KR')}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="lg:grid lg:grid-cols-12 lg:gap-16 items-start">
                        ...
                        <div className="lg:col-span-5 mb-16 lg:mb-0 lg:sticky lg:top-32">
                            ...
                            <p className="text-lg sm:text-xl text-gray-600 leading-relaxed mb-8 break-keep">
                                수수료의 거품은 빼고, 신뢰의 깊이는 더했습니다.<br className="hidden sm:block" />
                                100만 유저가 선택한 가장 빠르고 안전한<br className="hidden sm:block" />
                                O2O 플랫폼의 차이를 경험하세요.
                            </p>
                        </div>
                        ...
                        <div className="lg:col-span-7 space-y-8 sm:space-y-12">
                            ...
                            <div className="flex flex-col sm:flex-row gap-6 group">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600 text-lg leading-relaxed break-keep">
                                        모든 전문가는 신분증 및 연락처 인증을 통과한 안심 마스터들입니다. 낯선 사람을 부르는 불안감을 완벽하게 지웠습니다.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-6 group">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600 text-lg leading-relaxed break-keep">
                                        히든프로는 결제 중개 수수료를 단 1페소도 요구하지 않습니다. 고객과 고수 간의 1:1 채팅을 통해 투명하게 직거래하세요.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col sm:flex-row gap-6 group">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600 text-lg leading-relaxed break-keep">
                                        무분별한 알람과 연락은 그만. 가장 빠르고 검증된 상위 5명의 마스터 견적만 비교하여 고객님의 소중한 시간과 비용을 절약합니다.
                                    </p>
                                </div>
                            </div>
            ...
            <section className="relative py-48 md:py-64 bg-[#0B0F19] overflow-hidden text-center flex flex-col items-center justify-center">
                ...
                <div className="relative z-10 max-w-5xl mx-auto px-6">
                    <h2 className="text-[48px] md:text-[68px] font-black tracking-[-0.04em] text-white mb-10 leading-[1.3] break-keep">
                        전문가이신가요?<br />
                        <span className="text-[#FFD335]">HiddenPro</span>와 함께<br className="block sm:hidden" /> 새로운 고객을 만나보세요.
                    </h2>
                    <p className="text-[19px] md:text-[26px] text-gray-300 mb-20 max-w-4xl mx-auto font-medium leading-[1.85] tracking-[-0.02em] break-keep">
                        내 주변의 일거리를 가장 빠르고 편하게 확인하고<br className="hidden sm:block" />
                            ...
                            <div className="p-6 space-y-5">
                                ...
                                {authMode === 'pro_signup' && (
                                    <div className="bg-gray-900 text-yellow-400 p-4 rounded-xl text-center">
                                        <p className="text-sm font-bold">💰 전문가로 등록하고 새로운 고객을 만나보세요</p>
                                        <p className="text-xs text-gray-400 mt-1">가입 후 프로필을 완성하면 견적 발송이 가능합니다</p>
                                    </div>
                                )}
                                ...
                                <p className="text-[11px] text-gray-400 text-center leading-relaxed">
                                    계속 진행하면 HiddenPro의 <span className="underline cursor-pointer">이용약관</span> 및 <span className="underline cursor-pointer">개인정보처리방침</span>에 동의하는 것으로 간주됩니다.
                                </p>
                            </div>
                        </div>
                    </div>
