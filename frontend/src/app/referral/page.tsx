...
export default function ReferralPage() {
    ...
    useEffect(() => {
        const init = async () => {
            ...
            const { data: rewardsData } = await supabase
                ...
                .select('*, referred:referred_id(email, role), referrer:referrer_id(email, role)')
                .eq('referrer_id', uid)
    ...
    const handleShare = async () => {
        ...
        if (navigator.share) {
            try {
                await navigator.share({ title: 'HiddenPro', text: t('referral.inviteBanner'), url: link });
            } catch {}
        } else {
            handleCopyLink();
        }
        ...
        <div className="min-h-screen bg-gray-50 pb-24 pt-4">
            <div className="max-w-2xl mx-auto px-4 space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                    <h1 className="text-xl font-bold text-gray-900 mb-1">🎁 {t('referral.pageTitle')}</h1>
                    ...
                    <div className="bg-blue-50 rounded-xl py-3 px-6 inline-block mb-4">
                        <span className="text-2xl font-black text-blue-600 tracking-widest">{referralCode}</span>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
                        ...
                        <p className="text-xs text-gray-400 mt-1">{t('referral.statsTotal')}</p>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
                        <p className="text-2xl font-black text-green-600">{completedCount}</p>
                        <p className="text-xs text-gray-400 mt-1">{t('referral.statsCompleted')}</p>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 text-center">
                        <p className="text-2xl font-black text-yellow-500">{pendingCount}</p>
                        <p className="text-xs text-gray-400 mt-1">{t('referral.statsPending')}</p>
                    </div>
                </div>

                {/* C. 내 추천 리스트 */}
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <h2 className="text-sm font-bold text-gray-700 p-4 border-b border-gray-100">{t('referral.listTitle')}</h2>
                    {rewards.length === 0 ? (
                        <div className="p-8 text-center text-gray-400 text-sm">{t('referral.listEmpty')}</div>
                    ) : (
                        <ul className="divide-y divide-gray-50">
                            {rewards.map((r) => (
                                <li key={r.id} className="p-4 flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-gray-800">{maskEmail(r.referred?.email)}</p>
                                    ...
                                    <div className="text-right">
                                        {r.status === 'COMPLETED' ? (
                                            <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded-full">
                                                ✅ {r.referrer_reward_type === 'BONUS_CREDITS' ? `+${r.referrer_reward_amount} Credits` : `Coupon ${r.referrer_reward_amount}`}
                                            </span>
                                        ) : (
                ...
                {!isPro && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <h2 className="text-sm font-bold text-gray-700 p-4 border-b border-gray-100">🎟️ {t('referral.couponsTitle')}</h2>
                        {coupons.length === 0 ? (
                            <div className="p-8 text-center text-gray-400 text-sm">{t('referral.couponsEmpty')}</div>
                        ) : (
                            <ul className="divide-y divide-gray-50">
                                {coupons.map((c) => (
                                    <li key={c.coupon_id} className="p-4 flex items-center justify-between">
                                        <div>
                                            <p className="text-lg font-black text-blue-600">{c.discount_amount} {t('referral.couponOff')}</p>
                                            <p className="text-xs text-gray-400">{t('referral.couponCode')}: <span className="font-mono">{c.coupon_code}</span></p>
                                            <p className="text-xs text-gray-400">{t('referral.couponExpires')}: {new Date(c.expires_at).toLocaleDateString()}</p>
                                        </div>
                                        <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                                            ...
                                            {c.status === 'ACTIVE' ? t('referral.couponActive') :
                                             c.status === 'USED' ? t('referral.couponUsed') :
                ...
                {isPro && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        <h2 className="text-sm font-bold text-gray-700 mb-3">🎟️ {t('referral.redeemTitle')}</h2>
                    ...
                    {showHowItWorks && (
                        <div className="px-4 pb-4 space-y-3 text-sm text-gray-600 border-t border-gray-100 pt-3">
                            <div className="bg-blue-50 rounded-lg p-3">
                                <p className="font-bold text-blue-700 text-xs mb-1">🔧→🔧 Pro invites Pro</p>
                                <p>{t('referral.howProToPro')}</p>
                            </div>
                            <div className="bg-purple-50 rounded-lg p-3">
                                <p className="font-bold text-purple-700 text-xs mb-1">👤→🔧 Customer invites Pro</p>
                                <p>{t('referral.howCustToPro')}</p>
                            </div>
                            <div className="bg-green-50 rounded-lg p-3">
                                <p className="font-bold text-green-700 text-xs mb-1">👤→👤 Customer invites Customer</p>
                                <p>{t('referral.howCustToCust')}</p>
                            </div>
                            <div className="bg-orange-50 rounded-lg p-3">
                                <p className="font-bold text-orange-700 text-xs mb-1">🔧→👤 Pro invites Customer</p>
