...
export default function InquiryPage() {
    ...
    const fetchHistory = async () => {
        ...
        if (error) {
            ...
            showToast('문의 내역을 불러오는데 실패했습니다.', 'error');
        } else if (data) {
            setInquiries(data);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!category || !title.trim() || !content.trim()) {
            showToast('카테고리, 제목, 내용을 모두 입력해주세요.', 'error');
            return;
        }
        ...
        if ((todayCount ?? 0) >= 10) {
            showToast('하루 최대 10건까지 문의를 작성할 수 있습니다.', 'error');
            setIsSubmitting(false);
            return;
        }
        ...
        if (error) {
            ...
            showToast('문의 등록 중 오류가 발생했습니다: ' + error.message, 'error');
        } else {
    ...
    const StatusBadge = ({ status }: { status: string }) => {
        switch (status) {
            case 'pending':
                return <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2.5 py-1 rounded-full border border-yellow-200">접수 대기</span>;
            case 'in_progress':
                return <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-1 rounded-full border border-blue-200">처리 중</span>;
            case 'resolved':
                return <span className="bg-green-100 text-green-800 text-xs font-bold px-2.5 py-1 rounded-full border border-green-200">답변 완료</span>;
            default:
                return null;
        }
    };

    if (!user) return <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">로딩중...</div>;
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col pt-4 pb-20">
            <div className="flex justify-between items-center p-4 bg-white border-b border-gray-100 sticky top-0 z-10">
                ...
                <h1 className="font-bold text-gray-800">1:1 문의하기</h1>
            ...
            <main className="flex-1 w-full max-w-2xl mx-auto p-4 md:p-6 w-full">
                ...
                {activeTab === 'WRITE' && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        <p className="text-sm text-gray-500 mb-6 font-medium">서비스 이용 중 불편하신 점이나 궁금한 점을 남겨주시면 신속하게 답변해 드리겠습니다.</p>
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">문의 유형 <span className="text-red-500">*</span></label>
                                <select
                                    value={category}
                                    ...
                                    required
                                >
                                    <option value="" disabled>어떤 유형의 문의인가요?</option>
                            ...
                            <div>
                                <label className="block text-sm font-bold text-gray-700 mb-1">문의 내용 <span className="text-red-500">*</span></label>
                                <textarea
                                    value={content}
                                    onChange={(e) => setContent(e.target.value)}
                                    placeholder="자세한 문의 내용을 입력해주세요. (최소 10자 이상)"
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        {loadingHistory ? (
                            ...
                            <div className="p-12 text-center text-gray-400 flex flex-col items-center justify-center">
                                ...
                                <p className="font-medium">작성하신 문의 내역이 없습니다.</p>
                            </div>
                        ) : (
                                        ...
                                        <li key={iq.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition">
                                            ...
                                            {isExpanded && (
                                                <div className="bg-gray-50 p-5 border-t border-gray-100 shadow-inner">
                                                    <div className="mb-4">
                                                        <span className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">나의 질문</span>
                                                            ...
                                                            <div className="flex flex-wrap gap-2 mt-3">
                                                                {imgs.map((url: string, idx: number) => (
                                                                    <a key={idx} href={url} target="_blank" rel="noopener noreferrer">
                                                                        <img src={url} alt={`첨부 ${idx + 1}`} className="w-20 h-20 object-cover rounded-xl border border-gray-200 hover:opacity-80 transition" />
                                                                    </a>
                                                                ))}
                                                            </div>
                                                        )}
                                                    ...
                                                    {iq.status === 'resolved' && iq.admin_reply && (
                                                        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mt-4">
                                                            <div className="flex items-center gap-2 mb-2">
                                                                ...
                                                                <span className="text-xs font-bold text-blue-800 uppercase tracking-wider">관리자 답변</span>
                                                            </div>
                                                            <div
                                                            ...
                                                            {Array.isArray(iq.admin_reply_images) && iq.admin_reply_images.length > 0 && (
                                                                <div className="flex flex-wrap gap-2 mt-3">
                                                                    {iq.admin_reply_images.map((url: string, idx: number) => (
                                                                        <a key={idx} href={url} target="_blank" rel="noopener noreferrer">
                                                                            <img src={url} alt={`답변첨부${idx + 1}`} className="w-20 h-20 object-cover rounded-xl border border-blue-200 hover:opacity-80 transition" />
                                                                        </a>
                                                                    ))}
                                                                </div>
                                                            )}
