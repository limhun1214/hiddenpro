...
export default function ChatRoomPage({ params }: { params: { room_id: string } }) {
    ...
    const [reportStatus, setReportStatus] = useState<'none' | 'pending' | 'reviewed'>('none');
        ...
        const initChat = async () => {
            ...
            if (userId) {
                ...
                if (
                    ...
                    (suspendData?.suspended_until === null || new Date(suspendData.suspended_until) > new Date())
                ) {
                    setIsSuspended(true);
                }
                ...
                const { data: myReport } = await supabase
                    ...
                    .maybeSingle();
                if (myReport) {
                    setReportStatus(myReport.status === 'reviewed' ? 'reviewed' : 'pending');
                }
            ...
            if (roomData) {
                ...
                if (roomData.pro_id) {
                    const partnerId = (userId === roomData.pro_id) ? roomData.customer_id : roomData.pro_id;
            ...
            if (error) {
                ...
                const mappedMessages = sorted.map(msg => ({
                    ...
                    hidden: msg.message_type === 'SYSTEM_PRIVATE' && msg.receiver_id !== userId,
                }));

                const filtered = mappedMessages.filter(m => !m.hidden).map(m => ({ ...m, id: m.message_id }));
            ...
            messageChannel = supabase
                ...
                .on(
                    'postgres_changes',
                    { event: 'INSERT', schema: 'public', table: 'chat_messages', filter: `room_id=eq.${params.room_id}` },
                    (payload) => {
                        ...
                        setMessages((prev) => {
                            ...
                            const isDuplicate = prev.some(msg => msg.id === newMsg.message_id);
                            if (isDuplicate) return prev;
                            // SYSTEM_PRIVATE 메시지는 receiver_id가 본인일 때만 표시
                            const isHidden = newMsg.message_type === 'SYSTEM_PRIVATE' && newMsg.receiver_id !== userId;
                        ...
                        if (newMsg.content && newMsg.content.includes('진행하기로 확정하셨습니다')) {
                ...
                .on(
                    'postgres_changes',
                    { event: 'UPDATE', schema: 'public', table: 'chat_messages', filter: `room_id=eq.${params.room_id}` },
                    (payload) => {
                        const updatedMsg = payload.new as any;
                        setMessages((prev) =>
                            prev.map(msg => msg.id === updatedMsg.message_id ? { ...msg, is_read: updatedMsg.is_read } : msg)
                        );
                    }
                )
                .subscribe();
            ...
            statusChannel = supabase
                ...
                .on(
                    'postgres_changes',
                    { event: 'UPDATE', schema: 'public', table: 'match_requests', filter: `request_id=eq.${roomData?.request_id}` },
                    (payload) => {
    ...
    const loadMoreMessages = async () => {
        ...
        if (!error && data) {
            ...
            const mappedMessages = sorted.map(msg => ({
                ...
                hidden: msg.message_type === 'SYSTEM_PRIVATE' && msg.receiver_id !== currentUser?.id,
            })).filter(m => !m.hidden);
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        const clientMessageId = crypto.randomUUID ? crypto.randomUUID() : Array.from(crypto.getRandomValues(new Uint8Array(16))).map(b => b.toString(16).padStart(2, '0')).join('');
    ...
    const handleMatchConfirm = async () => {
        try {
            ...
            const { data, error } = await supabase.rpc('confirm_match_and_close_others', {
        ...
        <div className="relative w-full flex flex-col h-[calc(100dvh-72px)] lg:h-[100dvh] lg:max-w-full bg-white overflow-hidden">
            ...
            <header className="flex-none bg-white border-b border-gray-100 px-4 pt-3 pb-2 shadow-sm z-50">
                ...
                <div className="flex items-center">
                    ...
                    <div className="flex-shrink-0 mr-3">
                        {partnerAvatar ? (
                            <img src={partnerAvatar} alt="Profile" className="w-10 h-10 rounded-full object-cover border border-gray-200 shadow-sm" />
                        ) : (
                    ...
                    messages.map((msg, index) => {
                        ...
                        return (
                            <div key={msg.id} className="flex flex-col">
                                ...
                                {(msg.message_type === 'SYSTEM_PRIVATE' || msg.message_type === 'SYSTEM') ? (
                                ...
                                <div className={`flex ${msg.isMine ? 'justify-end' : 'justify-start'} mb-2`}>
                                    {!msg.isMine && (
                                        <div className="flex-shrink-0 mr-3 mt-1">
                                            {partnerAvatar ? (
                                                <img src={partnerAvatar} alt="Profile" className="w-9 h-9 rounded-full object-cover border border-gray-200 shadow-sm" />
                                            ) : (
                                    ...
                                    <div className={`flex flex-col ${msg.isMine ? 'items-end' : 'items-start'} max-w-[75%]`}>
                                        {!msg.isMine && <span className="text-xs text-gray-600 mb-1 ml-1 font-bold">{proName}</span>}

                                        <div className={`rounded-2xl px-4 py-2 shadow-sm ${msg.isMine ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white border border-gray-100 text-gray-800 rounded-tl-none'}`}>
                                            <p className="text-sm whitespace-pre-wrap leading-relaxed">{renderMessageWithLinks(msg.content, msg.isMine)}</p>
                                        </div>

                                        {/* 기존의 시간 및 읽음 표시 렌더링 영역 유지 */}
                                        <span className={`text-[10px] flex items-center gap-1 mt-1 ${msg.isMine ? 'text-gray-400 justify-end' : 'text-gray-400 justify-start'}`}>
                                            {msg.isMine && !msg.is_read && <span className="text-yellow-500 font-bold">1</span>}
            ...
            <div className="flex-none bg-white border-t border-gray-100 p-3">
                <form onSubmit={handleSendMessage} className="flex gap-2">
                    ...
                    <button
                        type="submit"
                        disabled={!newMessage.trim() || isSuspended || isBlockedInRoom || isRoomClosed}
