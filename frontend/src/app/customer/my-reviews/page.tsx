  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      const { data: rawReviews, error: reviewError } = await supabase
        .from("reviews")
        .select(
          "review_id, rating, comment, created_at, pro_id, room_id, customer_id",
        )
      ...
      if (!reviewError && rawReviews) {
        ...
        const proIds = Array.from(
          new Set(rawReviews.map((r: any) => r.pro_id)),
        );
        ...
        for (const pid of proIds) {
          const { data: userData } = await supabase
            ...
            .single();
          nameMap[pid] =
            userData?.nickname || userData?.name || t("myReviews.unknownPro");
