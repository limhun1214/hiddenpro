...
export default function ProProfileDetailModal({
  proId,
  requestId,
  hideChat,
  onClose,
  onStartChat,
}: ProProfileDetailModalProps) {
    ...
    const fetchData = async () => {
      ...
      setProName(
        profile?.nickname ||
          userData?.nickname ||
          userData?.name ||
          t("proProfileModal.defaultProName"),
      );
      ...
      if (reviewsData && reviewsData.length > 0) {
        ...
        if (customerUsers) {
          customerUsers.forEach((u: any) => {
            nameMap[u.user_id] = u.nickname || u.name;
          });
        }
        ...
        const enrichedReviews = reviewsData.map((r: any) => ({
          ...r,
          reviewer_name:
            nameMap[r.customer_id] || t("proProfileModal.unknownCustomer"),
        }));
  ...
  const services = Array.isArray(proProfile?.services)
    ? proProfile.services
    : [];
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden border border-gray-200">
                {avatarUrl ? (
                  <img
                    src={avatarUrl}
                    alt={t("proProfileModal.avatarAlt")}
              ...
              <h2 className="text-lg font-bold text-gray-800">
                {proName}
                {t("proProfileModal.profileTitle")}
              </h2>
            </div>
            <button
              onClick={onClose}
            ...
            {activeTab === "INFO" ? (
              <>
                  ...
                  {!proProfile?.is_phone_verified &&
                    !proProfile?.facebook_url && (
                      <span className="text-xs text-gray-400">
                        {t("proProfileModal.noVerification")}
                      </span>
                    )}
                </div>
                ...
                {proProfile?.detailed_intro && (
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                      {t("proProfileModal.detailedIntroTitle")}
                    </h3>
                ...
                {services.length > 0 && (
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                    <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                      {t("proProfileModal.servicesTitle")}
                    </h3>
                    <div className="flex flex-wrap gap-2">
                      {services.map((s: string) => (
                        <span
                          key={s}
                          className="bg-white text-gray-700 text-xs px-2.5 py-1.5 rounded-lg border border-gray-200 shadow-sm"
                        >
                          {s}
                        </span>
                      ))}
                ...
                {reviews.length === 0 ? (
                  ...
                  reviews.map((review: any) => (
                    <div
                      ...
                      className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-2"
                    >
                      ...
                      <p className="text-xs text-gray-400">
                        {review.reviewer_name ||
                          t("proProfileModal.unknownCustomer")}
                      </p>
                    </div>
                  ))
                )}
