...
export default function AdminReferralTab() {
  ...
  const [newBannerType, setNewBannerType] = useState<
    "horizontal" | "vertical" | "square" | "custom"
  >("horizontal");
  const [newBannerPlatform, setNewBannerPlatform] = useState(
    "Facebook, Viber, Blog",
  );
  ...
  const [subTab, setSubTab] = useState<
    "overview" | "referrals" | "coupons" | "banners"
  >("overview");
  ...
  const loadSettings = useCallback(async () => {
    ...
    if (data) {
      ...
      data
        .filter((s) => referralKeys.includes(s.key))
        .forEach((s: any) => {
  ...
  const loadReferrals = useCallback(async () => {
    ...
    let q = supabase
      .from("referral_rewards")
      .select(
        "*, referrer:referrer_id(email, role, name), referred:referred_id(email, role, name)",
      )
  ...
  useEffect(() => {
    if (subTab === "coupons") loadCoupons();
  }, [subTab, couponFilter]);
  useEffect(() => {
    if (subTab === "banners") loadBanners();
  }, [subTab]);
  ...
  const settingsConfig = [
    {
      ...
      t: "Referral System ON/OFF",
      d: "Set to 1 to enable, 0 to disable",
    },
    {
      key: "referral_bonus_credits",
      t: "Bonus Credits per Referral",
      d: "Credits given for successful pro referral",
    },
    {
      key: "referral_coupon_amount",
      t: "Coupon Discount Amount",
      d: "Discount amount for customer coupons",
    },
    {
      key: "coupon_expiry_days",
      t: "Coupon Expiry (Days)",
      d: "Days before coupon expires (new issues only)",
    },
    {
      key: "bonus_credit_expiry_days",
      t: "Bonus Credit Expiry (Days)",
      d: "Days before bonus credits expire (new issues only)",
    },
  ];
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <input
                    type="number"
                    value={
                      settingsInputs[s.key] ?? String(settings[s.key] ?? 0)
                    }
                    onChange={(e) =>
              ...
              <tbody>
                {rewardsLoading ? (
                  ...
                  rewards.map((r) => (
                    <tr
                      key={r.id}
                      className="border-b border-gray-700/50 hover:bg-gray-700/30 whitespace-nowrap"
                    >
                      ...
                      <td className="p-3 text-center text-xs text-gray-400">
                        {r.trigger_event === "FIRST_QUOTE_SENT"
                          ? "📋 1st Quote"
                          : "📝 1st Request"}
                      </td>
                      ...
                      <td className="p-3 text-right text-sm font-bold">
                        {r.referred_reward_type === "BONUS_CREDITS"
                          ? `+${r.referred_reward_amount} Credits`
                          : `Coupon ${r.referred_reward_amount}`}
                      </td>
                ...
                {couponsLoading ? (
                  ...
                  coupons.map((c) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <span
                          className={`text-xs font-bold px-2 py-0.5 rounded-full ${statusColor(c.status)}`}
                        >
                          {c.status}
                        </span>
                      </td>
                      <td className="p-3 text-gray-400">
                        {c.pro?.name || c.pro?.email?.split("@")[0] || "—"}
                      </td>
                      <td className="p-3 text-center text-xs text-gray-400">
                        {c.source}
                      </td>
        ...
        <>
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-5 mb-6 space-y-4">
            <h3 className="text-sm font-bold text-white">
              Upload New Banner (Max 4)
            </h3>
            {banners.length >= 4 ? (
              <p className="text-yellow-400 text-sm">
                Maximum 4 banners reached. Delete one to upload a new banner.
              </p>
            ) : (
              <>
          ...
          <div className="space-y-4">
            {bannersLoading ? (
              ...
              banners.map((b) => (
                <div
                  key={b.id}
                  className={`bg-gray-800 rounded-xl border ${b.is_default ? "border-yellow-500" : "border-gray-700"} overflow-hidden`}
                >
                  ...
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => toggleBannerDefault(b.id)}
                        className={`text-xs px-2 py-1 rounded font-bold ${b.is_default ? "bg-yellow-600 text-white" : "bg-gray-700 text-gray-400 hover:bg-gray-600"}`}
                      >
                        {b.is_default ? "⭐ Default" : "Set Default"}
                      </button>
                      <button
                        onClick={() => toggleBannerActive(b.id, b.is_active)}
                        className={`text-xs px-2 py-1 rounded font-bold ${b.is_active ? "bg-green-600 text-white" : "bg-gray-700 text-gray-400"}`}
                      >
                        {b.is_active ? "Active" : "Hidden"}
                      </button>
                      <button
                        onClick={() => deleteBanner(b.id)}
                        className="text-xs px-2 py-1 rounded font-bold bg-red-600 text-white hover:bg-red-500"
                      >
                        Delete
                      </button>
                    </div>
