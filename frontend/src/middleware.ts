...
export async function middleware(request: NextRequest) {
  ...
  const isProProfileRoute =
    pathname.startsWith("/pro/") &&
    uuidRegex.test(pathname.split("/")[2] || "");
  ...
  try {
    ...
    const status = String(
      user?.app_metadata?.status || user?.user_metadata?.status || "ACTIVE",
    ).toUpperCase();
    ...
    if (
      isAdminRoute &&
      !["ADMIN", "ADMIN_OPERATION", "ADMIN_VIEWER"].includes(role)
    ) {
