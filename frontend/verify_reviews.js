...
async function testReviewSystem() {
  ...
  const { data: room, error: roomErr } = await supabase
    .from("chat_rooms")
    .select("room_id, pro_id, customer_id")
    .limit(1)
    .single();
  if (roomErr || !room) {
  ...
  console.log(
    `Testing with Room: ${room_id}, Pro: ${pro_id}, Customer: ${customer_id}`,
  );
  ...
  const { data: initialProfile } = await supabase
    ...
    .single();
  console.log(
    `Initial Profile -> Rating: ${initialProfile.average_rating}, Count: ${initialProfile.review_count}`,
  );
  ...
  const { data: updatedProfile1 } = await supabase
    .from("pro_profiles")
    .select("average_rating, review_count")
    .eq("pro_id", pro_id)
    .single();
  console.log(
    `Profile after 1st review -> Rating: ${updatedProfile1.average_rating}, Count: ${updatedProfile1.review_count}`,
  );
  ...
  if (duplicateErr) {
    console.log(
      `Duplicate blocked successfully: [${duplicateErr.code}] ${duplicateErr.message}`,
    );
  } else {
  ...
  const { data: finalProfile } = await supabase
    ...
    .single();
  console.log(
    `Final Profile after cleanup -> Rating: ${finalProfile.average_rating}, Count: ${finalProfile.review_count}`,
  );
