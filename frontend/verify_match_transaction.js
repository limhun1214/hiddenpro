...
async function test() {
  ...
  console.log(
    "Simulating handleAcceptQuote execution on quote:",
    winningQuote.quote_id,
  );
  ...
  await supabase
    ...
    .eq("quote_id", winningQuote.quote_id);
  await supabase
    .from("match_quotes")
    .update({ status: "REJECTED" })
    .eq("request_id", reqId)
    .neq("quote_id", winningQuote.quote_id);
  ...
  const { data: verReq } = await supabase
    ...
    .eq("request_id", reqId)
    .single();
  const { data: verWin } = await supabase
    .from("match_quotes")
    .select("status")
    .eq("quote_id", winningQuote.quote_id)
    .single();
  const { data: verLose } = await supabase
    .from("match_quotes")
    .select("status")
    .eq("quote_id", losingQuote.quote_id)
    .single();
  ...
  if (
    verReq.status === "MATCHED" &&
    verWin.status === "ACCEPTED" &&
    verLose.status === "REJECTED"
  ) {
