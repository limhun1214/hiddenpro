...
async function test() {
    console.log('Validating UI rendering logic changes indirectly via database state checking...');

    // We already inserted a winning quote in the previous step. Let's find it.
    const { data: quote } = await supabase.from('match_quotes').select('request_id, pro_id, status').eq('status', 'ACCEPTED').limit(1).single();
    if (!quote) return console.log('No accepted quotes found. Cannot test.');

    const { data: req } = await supabase.from('match_requests').select('status, created_at').eq('request_id', quote.request_id).single();
    ...
    if (isDisabled === false) {
        console.log('SUCCESS: The View Details button is ENABLED for the accepted Pro.');
    } else {
